...
/* Setting _FILE_OFFSET_BITS to 64 doesn't change lseek to use off64_t on Windows, so override lseek
...
#define DCC_USE_PROXY()                                                                            \
        (prefs.proxy_host[0] && prefs.proxy_type > 0 && prefs.proxy_type < 5 &&                    \
         prefs.proxy_use != 1)
        ...
        if (dcc->fp != -1) {
                ...
                if (dccstat == STAT_DONE) {
                        ...
                        if (dcc->type == TYPE_RECV) {
                                ...
                                move_file_utf8(prefs.dccdir,
                                               prefs.dcc_completed_dir,
...
void dcc_abort(session *sess, struct DCC *dcc)
{
        if (dcc) {
                ...
                case STAT_ACTIVE:
                        ...
                        case TYPE_CHATSEND:
                        case TYPE_CHATRECV:
                                EMIT_SIGNAL(XP_TE_DCCCHATABORT,
                                            sess,
                                            dcc->nick,
                                            NULL,
                                            NULL,
                                ...
                                break;
                        case TYPE_SEND:
                                EMIT_SIGNAL(XP_TE_DCCSENDABORT,
                                            sess,
                                            dcc->nick,
                                            file_part(dcc->file),
                                            NULL,
                                ...
                                break;
                        case TYPE_RECV:
                                EMIT_SIGNAL(XP_TE_DCCRECVABORT,
                                            sess,
                                            dcc->nick,
                                            dcc->file,
                                            NULL,
...
struct DCC *dcc_write_chat(char *nick, char *text)
{
        ...
        if (dcc && dcc->dccstat == STAT_ACTIVE) {
                len = strlen(text);
                tcp_send_real(NULL, dcc->sok, dcc->serv->encoding, dcc->serv->using_irc, text, len);
                send(dcc->sok, "\n", 1, 0);
                dcc->size += len;
                fe_dcc_update(dcc);
                return dcc;
...
static gboolean dcc_read_chat(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        int i, len, dead;
        char portbuf[32];
        char lbuf[2050];
        ...
        while (1) {
                ...
                while (i < len) {
                        ...
                        case '\n':
                                ...
                                if (dead || !dcc->dccchat) /* the dcc has been closed, don't use
                                                              (DCC *)! */
                                        return TRUE;
...
static gboolean dcc_read(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        char *old;
        char buf[4096];
        int n;
        ...
        while (1) {
                ...
                if (n < 1) {
                        ...
                        EMIT_SIGNAL(XP_TE_DCCRECVERR,
                                    dcc->serv->front_session,
                                    dcc->file,
                                    dcc->destfile,
                                    dcc->nick,
                                    errorstring((n < 0) ? sock_error() : 0),
                                    0);
                ...
                if (dcc->pos >= dcc->size) {
                        ...
                        dcc_calc_average_cps(dcc); /* this must be done _after_ dcc_close, or
                                                      dcc_remove_from_sum will see the wrong value
                                                      in dcc->cps */
                        sprintf(buf, "%d", dcc->cps);
                        EMIT_SIGNAL(XP_TE_DCCRECVCOMP,
                                    dcc->serv->front_session,
                                    dcc->file,
                                    dcc->destfile,
                                    dcc->nick,
...
static gboolean dcc_did_connect(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        int er;
        struct sockaddr_in addr;
...
static gboolean dcc_connect_finished(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        char host[128];
...
static gboolean read_proxy(struct DCC *dcc)
{
        ...
        while (proxy->bufferused < proxy->buffersize) {
                int ret = recv(dcc->sok,
                               &proxy->buffer[proxy->bufferused],
                               proxy->buffersize - proxy->bufferused,
                               0);
                if (ret > 0)
...
static gboolean dcc_wingate_proxy_traverse(GIOChannel *source, GIOCondition condition,
                                           struct DCC *dcc)
{
        struct proxy_state *proxy = dcc->proxy;
        if (proxy->phase == 0) {
                proxy->buffersize = snprintf((char *)proxy->buffer,
                                             MAX_PROXY_BUFFER,
                                             "%s %d\r\n",
                                             net_ip(dcc->addr),
                                             dcc->port);
                proxy->bufferused = 0;
                dcc->wiotag =
                    fe_input_add(dcc->sok, FIA_WRITE | FIA_EX, dcc_wingate_proxy_traverse, dcc);
                ++proxy->phase;
        }
        if (proxy->phase == 1) {
                if (!read_proxy(dcc))
...
static gboolean dcc_socks_proxy_traverse(GIOChannel *source, GIOCondition condition,
                                         struct DCC *dcc)
{
        struct proxy_state *proxy = dcc->proxy;
        ...
        if (proxy->phase == 0) {
                ...
                proxy->bufferused = 0;
                dcc->wiotag =
                    fe_input_add(dcc->sok, FIA_WRITE | FIA_EX, dcc_socks_proxy_traverse, dcc);
                ++proxy->phase;
        }
        ...
        if (proxy->phase == 1) {
                ...
                proxy->buffersize = 8;
                dcc->iotag =
                    fe_input_add(dcc->sok, FIA_READ | FIA_EX, dcc_socks_proxy_traverse, dcc);
                ++proxy->phase;
        }
...
static gboolean dcc_socks5_proxy_traverse(GIOChannel *source, GIOCondition condition,
                                          struct DCC *dcc)
{
        struct proxy_state *proxy = dcc->proxy;
        ...
        if (proxy->phase == 0) {
                ...
                proxy->bufferused = 0;
                dcc->wiotag =
                    fe_input_add(dcc->sok, FIA_WRITE | FIA_EX, dcc_socks5_proxy_traverse, dcc);
                ++proxy->phase;
        }
        ...
        if (proxy->phase == 1) {
                ...
                proxy->buffersize = 2;
                dcc->iotag =
                    fe_input_add(dcc->sok, FIA_READ | FIA_EX, dcc_socks5_proxy_traverse, dcc);
                ++proxy->phase;
        }
        ...
        if (proxy->phase == 2) {
                ...
                if (auth) {
                        ...
                        proxy->bufferused = 0;
                        dcc->wiotag = fe_input_add(dcc->sok,
                                                   FIA_WRITE | FIA_EX,
...
static gboolean dcc_http_proxy_traverse(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        struct proxy_state *proxy = dcc->proxy;
        ...
        if (proxy->phase == 0) {
                ...
                dcc->wiotag =
                    fe_input_add(dcc->sok, FIA_WRITE | FIA_EX, dcc_http_proxy_traverse, dcc);
                ++proxy->phase;
        }
...
static gboolean dcc_proxy_connect(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        fe_input_remove(dcc->iotag);
        dcc->iotag = 0;
...
static gboolean dcc_send_data(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        char *buf;
        ...
        abortit:
                free(buf);
                EMIT_SIGNAL(XP_TE_DCCSENDFAIL,
                            dcc->serv->front_session,
                            file_part(dcc->file),
                            dcc->nick,
                            errorstring(sock_error()),
                            NULL,
                            0);
                dcc_close(dcc, STAT_FAILED, FALSE);
                return TRUE;
...
static gboolean dcc_handle_new_ack(struct DCC *dcc)
{
        ...
        if (dcc->pos >= dcc->size && dcc->ack >= (dcc->size & 0xffffffff)) {
                ...
                dcc_close(dcc, STAT_DONE, FALSE);
                dcc_calc_average_cps(dcc); /* this must be done _after_ dcc_close, or
                                              dcc_remove_from_sum will see the wrong value in
                                              dcc->cps */
                sprintf(buf, "%d", dcc->cps);
                EMIT_SIGNAL(XP_TE_DCCSENDCOMP,
                            dcc->serv->front_session,
                            file_part(dcc->file),
                            dcc->nick,
                            buf,
        ...
        dcc->ack = (dcc->pos & G_GINT64_CONSTANT(0xffffffff00000000)) | (dcc->ack & 0xffffffff);
...
static gboolean dcc_accept(GIOChannel *source, GIOCondition condition, struct DCC *dcc)
{
        char host[128];
...
guint32 dcc_get_my_address(void) /* the address we'll tell the other person */
{
...
static int dcc_listen_init(struct DCC *dcc, session *sess)
{
        ...
        if (prefs.first_dcc_send_port > 0) {
                SAddr.sin_port = 0;
                i = 0;
                while ((prefs.last_dcc_send_port > ntohs(SAddr.sin_port)) && (bindretval == -1)) {
                        ...
                        if (passive || dcc_listen_init(dcc, sess)) {
                                ...
                                if (passive) {
                                        ...
                                        snprintf(outbuf,
                                                 sizeof(outbuf),
                                                 (havespaces)
                                                     ? "DCC SEND \"%s\" 199 0 %" G_GUINT64_FORMAT
                                                       " %d"
...
static int is_same_file(struct DCC *dcc, struct DCC *new_dcc)
{
...
static int is_resumable(struct DCC *dcc)
{
        dcc->resumable = 0;
...
int dcc_resume(struct DCC *dcc)
{
        ...
        if (dcc->dccstat == STAT_QUEUED && dcc->resumable) {
                ...
                snprintf(tbuf,
                         sizeof(tbuf) - 10,
                         strchr(dcc->file, ' ') ? "DCC RESUME \"%s\" %d %" G_GUINT64_FORMAT
...
static struct DCC *dcc_add_chat(session *sess, char *nick, int port, guint32 addr, int pasvid)
{
        struct DCC *dcc;
...
static struct DCC *dcc_add_file(session *sess, char *file, guint64 size, int port, char *nick,
                                guint32 addr, int pasvid)
{
        ...
        if (dcc) {
                ...
                dcc->destfile = g_malloc(strlen(prefs.dccdir) + strlen(nick) + strlen(file) + 4);
        ...
        EMIT_SIGNAL(XP_TE_DCCSENDOFFER,
                    sess->server->front_session,
                    nick,
                    file,
                    tbuf,
...
void handle_dcc(struct session *sess, char *nick, char *word[], char *word_eol[])
{
        char tbuf[512];
        struct DCC *dcc;
        char *type = word[5];
        ...
        if (!strcasecmp(type, "Resume")) {
                ...
                if (dcc) {
                        ...
                        EMIT_SIGNAL(XP_TE_DCCRESUMEREQUEST,
                                    sess,
                                    nick,
                                    file_part(dcc->file),
                                    tbuf,
        ...
        if (!strcasecmp(type, "SEND")) {
                ...
                EMIT_SIGNAL(XP_TE_DCCGENERICOFFER,
                            sess->server->front_session,
                            word_eol[4] + 2,
                            nick,
                            NULL,
...
void dcc_show_list(struct session *sess)
{
        ...
        while (list) {
                ...
                i++;
                PrintTextf(sess,
                           " %s  %-10.10s %-7.7s %-7" G_GUINT64_FORMAT " %-7" G_GUINT64_FORMAT
                           " %s\n",
                           dcctypes[dcc->type],
                           dcc->nick,
