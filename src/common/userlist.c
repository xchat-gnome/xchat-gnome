...
static int nick_cmp_az_ops(server *serv, struct User *user1, struct User *user2)
{
...
static int nick_cmp_alpha(struct User *user1, struct User *user2, server *serv)
{
        return serv->p_cmp(user1->nick, user2->nick);
}

static int nick_cmp(struct User *user1, struct User *user2, server *serv)
{
...
static int userlist_insertname(session *sess, struct User *newuser)
{
        if (!sess->usertree) {
...
int userlist_add_hostname(struct session *sess, char *nick, char *hostname, char *realname,
...
static int free_user(struct User *user, gpointer data)
{
        if (user->realname)
                free(user->realname);
        if (user->hostname)
...
static int find_cmp(const char *name, struct User *user, server *serv)
{
        return serv->p_cmp((char *)name, user->nick);
}

struct User *userlist_find(struct session *sess, char *name)
{
        int pos;
...
struct User *userlist_find_global(struct server *serv, char *name)
{
        struct User *user;
        session *sess;
...
static void update_counts(session *sess, struct User *user, char prefix, int level, int offset)
{
        switch (prefix) {
        case '@':
                user->op = level;
...
int userlist_change(struct session *sess, char *oldname, char *newname)
{
...
int userlist_remove(struct session *sess, char *name)
{
        struct User *user;
        int pos;
...
void userlist_add(struct session *sess, char *name, char *hostname)
{
        struct User *user;
...
static int rehash_cb(struct User *user, session *sess)
{
        fe_userlist_rehash(sess, user);
        return TRUE;
}
...
static int flat_cb(struct User *user, GSList **list)
{
...
static int double_cb(struct User *user, GList **list)
{
