...
enum { SERVER_SIGNAL, COMMAND_SIGNAL, PRINT_SIGNAL, UNLOAD_SIGNAL, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = { 0 };

#define REMOTE_TYPE_OBJECT (remote_object_get_type())
#define REMOTE_OBJECT(object)                                                                      \
        (G_TYPE_CHECK_INSTANCE_CAST((object), REMOTE_TYPE_OBJECT, RemoteObject))
#define REMOTE_OBJECT_CLASS(klass)                                                                 \
        (G_TYPE_CHECK_CLASS_CAST((klass), REMOTE_TYPE_OBJECT, RemoteObjectClass))
...
#define REMOTE_OBJECT_GET_CLASS(obj)                                                               \
        (G_TYPE_INSTANCE_GET_CLASS((obj), REMOTE_TYPE_OBJECT, RemoteObjectClass))
...
static gboolean remote_object_connect(RemoteObject *obj, const char *filename, const char *name,
                                      const char *desc, const char *version,
                                      DBusGMethodInvocation *context);

static gboolean remote_object_disconnect(RemoteObject *obj, DBusGMethodInvocation *context);
...
static gboolean remote_object_find_context(RemoteObject *obj, const char *server,
                                           const char *channel, guint *ret_id, GError **error);

static gboolean remote_object_get_context(RemoteObject *obj, guint *ret_id, GError **error);

static gboolean remote_object_set_context(RemoteObject *obj, guint id, gboolean *ret,
                                          GError **error);

static gboolean remote_object_print(RemoteObject *obj, const char *text, GError **error);

static gboolean remote_object_get_info(RemoteObject *obj, const char *id, char **ret_info,
                                       GError **error);

static gboolean remote_object_get_prefs(RemoteObject *obj, const char *name, int *ret_type,
                                        char **ret_str, int *ret_int, GError **error);

static gboolean remote_object_hook_command(RemoteObject *obj, const char *name, int pri,
                                           const char *help_text, int return_value, guint *ret_id,
                                           GError **error);

static gboolean remote_object_hook_server(RemoteObject *obj, const char *name, int pri,
                                          int return_value, guint *ret_id, GError **error);

static gboolean remote_object_hook_print(RemoteObject *obj, const char *name, int pri,
                                         int return_value, guint *ret_id, GError **error);

static gboolean remote_object_unhook(RemoteObject *obj, guint id, GError **error);

static gboolean remote_object_list_get(RemoteObject *obj, const char *name, guint *ret_id,
                                       GError **error);

static gboolean remote_object_list_next(RemoteObject *obj, guint id, gboolean *ret, GError **error);

static gboolean remote_object_list_str(RemoteObject *obj, guint id, const char *name,
                                       char **ret_str, GError **error);

static gboolean remote_object_list_int(RemoteObject *obj, guint id, const char *name, int *ret_int,
                                       GError **error);

static gboolean remote_object_list_time(RemoteObject *obj, guint id, const char *name,
                                        guint64 *ret_time, GError **error);

static gboolean remote_object_list_fields(RemoteObject *obj, const char *name, char ***ret,
                                          GError **error);

static gboolean remote_object_list_free(RemoteObject *obj, guint id, GError **error);

static gboolean remote_object_emit_print(RemoteObject *obj, const char *event_name,
                                         const char *args[], gboolean *ret, GError **error);

static gboolean remote_object_nickcmp(RemoteObject *obj, const char *nick1, const char *nick2,
                                      int *ret, GError **error);

static gboolean remote_object_strip(RemoteObject *obj, const char *str, int len, int flag,
                                    char **ret_str, GError **error);

static gboolean remote_object_send_modes(RemoteObject *obj, const char *targets[],
                                         int modes_per_line, char sign, char mode, GError **error);
...
static void list_info_destroy(gpointer data)
{
        xchat_list_free(ph, (xchat_list *)data);
}

static void remote_object_finalize(GObject *obj)
{
...
static void remote_object_init(RemoteObject *obj)
{
...
static void remote_object_class_init(RemoteObjectClass *klass)
{
        ...
        signals[UNLOAD_SIGNAL] = g_signal_new("unload_signal",
                                              G_OBJECT_CLASS_TYPE(klass),
                                              G_SIGNAL_RUN_LAST,
                                              0,
                                              NULL,
                                      ...
                                      DBusGMethodInvocation *context)
{
        ...
        remote_object->handle =
            xchat_plugingui_add(ph, remote_object->filename, name, desc, version, NULL);
        dbus_g_connection_register_g_object(connection, path, G_OBJECT(remote_object));
...
static int server_hook_cb(char *word[], char *word_eol[], void *userdata)
{
...
static int command_hook_cb(char *word[], char *word_eol[], void *userdata)
{
        HookInfo *info = (HookInfo *)userdata;
        char **arg1;
        char **arg2;
...
static int print_hook_cb(char *word[], void *userdata)
{
        HookInfo *info = (HookInfo *)userdata;
        char **arg1;
...
static gboolean remote_object_hook_command(RemoteObject *obj, const char *name, int priority,
                                           const char *help_text, int return_value, guint *ret_id,
                                           GError **error)
{
        HookInfo *info;
        ...
        info->hook = xchat_hook_command(ph, name, priority, command_hook_cb, help_text, info);
...
static gboolean remote_object_hook_server(RemoteObject *obj, const char *name, int priority,
                                          int return_value, guint *ret_id, GError **error)
{
        HookInfo *info;
...
static gboolean remote_object_hook_print(RemoteObject *obj, const char *name, int priority,
                                         int return_value, guint *ret_id, GError **error)
{
        HookInfo *info;
                                         ...
                                         const char *args[], gboolean *ret, GError **error)
{
        ...
        *ret = xchat_set_context(ph, obj->context);
        if (*ret) {
                *ret = xchat_emit_print(ph, event_name, argv[0], argv[1], argv[2], argv[3]);
        }
...
static void name_owner_changed(DBusGProxy *driver_proxy, const char *name, const char *old_owner,
...
static int open_context_cb(char *word[], void *userdata)
{
        ContextInfo *info;
...
static int close_context_cb(char *word[], void *userdata)
{
        GList *l;
...
static gboolean clients_find_filename_foreach(gpointer key, gpointer value, gpointer user_data)
{
        RemoteObject *obj = REMOTE_OBJECT(value);
        return g_str_equal(obj->filename, (char *)user_data);
}

static int unload_plugin_cb(char *word[], char *word_eol[], void *userdata)
{
        RemoteObject *obj;
...
int dbus_plugin_init(xchat_plugin *plugin_handle, char **plugin_name, char **plugin_desc,
                     char **plugin_version, char *arg)
{
        ...
        *plugin_version = PVERSION;
        ...
        if (init_dbus()) {
                ...
                xchat_hook_print(ph, "Open Context", XCHAT_PRI_NORM, open_context_cb, NULL);

                xchat_hook_print(ph, "Close Context", XCHAT_PRI_NORM, close_context_cb, NULL);

                xchat_hook_command(ph, "unload", XCHAT_PRI_HIGHEST, unload_plugin_cb, NULL, NULL);
        }
