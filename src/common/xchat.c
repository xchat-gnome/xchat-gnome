...
static void lagcheck_update(void)
{
        server *serv;
...
doover:
        ...
        while (list) {
                ...
                if (sess->server->connected && sess->type == SESS_CHANNEL && sess->channel[0] &&
...
static int xchat_misc_checks(void) /* this gets called every 1/2 second */
{
        static int count = 0;
#ifdef USE_MSPROXY
        static int count2 = 0;
...
static void irc_init(session *sess)
{
...
static session *session_new(server *serv, char *from, int type, int focus)
{
        session *sess;
...
static void exec_notify_kill(session *sess)
{
#ifndef WIN32
        struct nbexec *re;
...
static void send_quit_or_part(session *killsess)
{
        int willquit = TRUE;
        GSList *list;
        session *sess;
...
static void sigusr1_handler(int signal, siginfo_t *si, void *un)
{
        GSList *list = sess_list;
        session *sess;
...
static void sigusr2_handler(int signal, siginfo_t *si, void *un)
{
...
static gint xchat_auto_connect(gpointer userdata)
{
        servlist_auto_connect(NULL);
        return 0;
}

static void xchat_init(void)
{
        char buf[3068];
        const char *cs = NULL;
...
static int child_handler(gpointer userdata)
{
