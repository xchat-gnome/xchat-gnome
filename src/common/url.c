...
static int url_free(char *url, void *data)
{
        free(url);
        return TRUE;
}
...
static int url_save_cb(char *url, FILE *fd)
{
        fprintf(fd, "%s\n", url);
        return TRUE;
}
...
static void url_add(char *urltext, int len)
{
...
#define D(x) (x), ((sizeof(x)) - 1)
        static const struct {
            ...
            {
              { D("irc.") },    { D("ftp.") },      { D("www.") },     { D("irc://") },
              { D("ftp://") },  { D("http://") },   { D("https://") }, { D("file://") },
              { D("rtsp://") }, { D("ut2004://") },
            },
          suffix[] = {
                  { D(".org") },  { D(".net") },  { D(".com") },  { D(".edu") },
                  { D(".html") }, { D(".info") }, { D(".name") },
          };
#undef D
        const char *at, *dot;
        int i, dots;
        ...
        if (len > 5) {
                ...
                if (word[len - 3] == '.' && isalpha((unsigned char)word[len - 2]) &&
