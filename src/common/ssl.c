...
static void __SSL_fill_err_buf(char *funcname)
{
        int err;
        char buf[256];
...
static void __SSL_critical_error(char *funcname)
{
...
SSL_CTX *_SSL_context_init(void(*info_cb_func), int server)
{
        ...
        SSL_CTX_set_options(ctx,
                            SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_COMPRESSION |
                                SSL_OP_SINGLE_DH_USE | SSL_OP_SINGLE_ECDH_USE | SSL_OP_NO_TICKET |
                                SSL_OP_CIPHER_SERVER_PREFERENCE);

#if OPENSSL_VERSION_NUMBER >= 0x00908000L && !defined(OPENSSL_NO_COMP) /* workaround for OpenSSL   \
                                                                          0.9.8 */
...
static void ASN1_TIME_snprintf(char *buf, int buf_len, ASN1_TIME *tm)
{
        char *expires = NULL;
...
static void broke_oneline(char *oneline, char *parray[])
{
        char *pt, *ppt;
        int i;
...
int _SSL_get_cert_info(struct cert_info *cert_info, SSL *ssl)
{
        ...
        ASN1_TIME_snprintf(notBefore, sizeof(notBefore), X509_get_notBefore(peer_cert));
        ASN1_TIME_snprintf(notAfter, sizeof(notAfter), X509_get_notAfter(peer_cert));
...
struct chiper_info *_SSL_get_cipher_info(SSL *ssl)
{
        ...
        safe_strcpy(chiper_info.version, SSL_CIPHER_get_version(c), sizeof(chiper_info.version));
        safe_strcpy(chiper_info.chiper, SSL_CIPHER_get_name(c), sizeof(chiper_info.chiper));
