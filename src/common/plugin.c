...
enum { LIST_CHANNELS, LIST_DCC, LIST_IGNORE, LIST_NOTIFY, LIST_USERS };

enum { HOOK_COMMAND, /* /command */
...
void plugin_add(session *sess, char *filename, void *handle, void *init_func, void *deinit_func,
                char *arg, int fake)
{
        xchat_plugin *pl;
        char *file;
        ...
        pl = plugin_list_add(sess, file, file, NULL, NULL, handle, deinit_func, fake, FALSE);
        ...
        if (!fake) {
                ...
                if (((xchat_init_func *)init_func)(pl, &pl->name, &pl->desc, &pl->version, arg) ==
                    0) {
...
int plugin_emit_keypress(session *sess, unsigned int state, unsigned int keyval, int len,
                         char *string)
{
        char *word[PDIWORDS];
        char keyval_str[16];
...
static gboolean plugin_fd_cb(GIOChannel *source, GIOCondition condition, xchat_hook *hook)
{
        int flags = 0, ret;
...
xchat_hook *xchat_hook_command(xchat_plugin *ph, const char *name, int pri, xchat_cmd_cb *callb,
                               const char *help_text, void *userdata)
{
        return plugin_add_hook(ph, HOOK_COMMAND, pri, name, help_text, callb, 0, userdata);
}

xchat_hook *xchat_hook_server(xchat_plugin *ph, const char *name, int pri, xchat_serv_cb *callb,
                              void *userdata)
{
        return plugin_add_hook(ph, HOOK_SERVER, pri, name, 0, callb, 0, userdata);
}

xchat_hook *xchat_hook_print(xchat_plugin *ph, const char *name, int pri, xchat_print_cb *callb,
                             void *userdata)
{
...
xchat_hook *xchat_hook_fd(xchat_plugin *ph, int fd, int flags, xchat_fd_cb *callb, void *userdata)
{
        xchat_hook *hook;
...
void xchat_command(xchat_plugin *ph, const char *command)
{
        ...
        if (!is_session(ph->context)) {
                DEBUG(
                    PrintTextf(0, "%s\txchat_command called without a valid context.\n", ph->name));
                return;
        }
...
xchat_context *xchat_find_context(xchat_plugin *ph, const char *servname, const char *channel)
{
        ...
        while (slist) {
                ...
                if (servname == NULL || rfc_casecmp(servname, serv->servername) == 0 ||
...
const char *xchat_get_info(xchat_plugin *ph, const char *id)
{
        ...
        if (!is_session(sess)) {
                DEBUG(PrintTextf(0,
                                 "%s\txchat_get_info called without a valid context.\n",
                                 ph->name));
                return NULL;
        }
...
int xchat_list_next(xchat_plugin *ph, xchat_list *xlist)
{
        ...
        if (xlist->type == LIST_NOTIFY) {
                xlist->notifyps =
                    notify_find_server_entry(xlist->pos->data, ((session *)xlist->head)->server);
                if (!xlist->notifyps)
                        return 0;
        }
...
{
        static const char *const dcc_fields[] = { "iaddress32", "icps",      "sdestfile", "sfile",
                                                  "snick",      "iport",     "ipos",      "iresume",
                                                  "isize",      "isizehigh", "istatus",   "itype",
                                                  NULL };
        static const char *const channels_fields[] = { "schannel",      "schantypes", "pcontext",
                                                       "iflags",        "iid",        "ilag",
                                                       "imaxmodes",     "snetwork",   "snickmodes",
                                                       "snickprefixes", "iqueue",     "sserver",
                                                       "itype",         "iusers",     NULL };
        static const char *const ignore_fields[] = { "iflags", "smask", NULL };
        static const char *const notify_fields[] = { "iflags", "snetworks", "snick", "toff",
                                                     "ton",    "tseen",     NULL };
        static const char *const users_fields[] = { "iaway",   "shost",     "tlasttalk", "snick",
                                                    "sprefix", "iselected", NULL };
        static const char *const list_of_lists[] = { "channels", "dcc",   "ignore",
                                                     "notify",   "users", NULL };
...
void *xchat_plugingui_add(xchat_plugin *ph, const char *filename, const char *name,
                          const char *desc, const char *version, char *reserved)
{
#ifdef USE_PLUGIN
        ph = plugin_list_add(NULL,
                             strdup(filename),
                             strdup(name),
                             strdup(desc),
                             strdup(version),
...
int xchat_emit_print(xchat_plugin *ph, const char *event_name, ...)
{
        ...
        i = text_emit_by_name((char *)event_name, ph->context, argv[0], argv[1], argv[2], argv[3]);
        va_end(args);
...
void xchat_send_modes(xchat_plugin *ph, const char **targets, int ntargets, int modes_per_line,
                      char sign, char mode)
{
        char tbuf[514]; /* modes.c needs 512 + null */

        send_channel_modes(ph->context,
                           tbuf,
                           (char **)targets,
                           0,
                           ntargets,
