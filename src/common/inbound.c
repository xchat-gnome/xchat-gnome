...
static int SearchNick(char *text, char *nicks)
{
        ...
        while (n != NULL) {
                ...
                while ((p = nocasestrstr(t, n))) {
                        ...
                        char *next_char = p + ns;
                        if ((!prev_char || !g_unichar_isalnum(g_utf8_get_char(prev_char))) &&
...
static int is_hilight(char *from, char *text, session *sess, server *serv)
{
        ...
        if (SearchNick(text, serv->nick) || SearchNick(text, prefs.irc_extra_hilight) ||
...
static session *find_unused_session(server *serv)
{
        ...
        while (list) {
                ...
                if (sess->type == SESS_CHANNEL && sess->channel[0] == 0 && sess->server == serv) {
...
void inbound_nameslist(server *serv, char *chan, char *names)
{
        ...
        sess = find_channel(serv, chan);
        if (!sess) {
                EMIT_SIGNAL(XP_TE_USERSONCHAN, serv->server_session, chan, names, NULL, NULL, 0);
                return;
        }
...
static gboolean check_willjoin_channels(server *serv)
{
        char *po;
        session *sess;
        GSList *list = sess_list;
...
static void inbound_set_all_away_status(server *serv, char *nick, unsigned int status)
{
        GSList *list;
        session *sess;
...
int inbound_user_info(session *sess, char *chan, char *user, char *host, char *servname, char *nick,
                      char *realname, unsigned int away)
{
        server *serv = sess->server;
        session *who_sess;
        char *uhost;
        ...
        if (who_sess) {
                if (user && host) {
                        ...
                        if (!userlist_add_hostname(who_sess,
                                                   nick,
                                                   uhost,
                                                   realname,
                                                   servname,
...
int inbound_banlist(session *sess, time_t stamp, char *chan, char *mask, char *banner,
                    int is_exemption)
{
...
void inbound_login_end(session *sess, char *text)
{
        ...
        if (!serv->end_of_motd) {
                ...
                if (serv->network && ((ircnet *)serv->network)->nickserv && prefs.irc_join_delay)
                        serv->joindelay_tag = fe_timeout_add(prefs.irc_join_delay * 1000,
                                                             check_willjoin_channels,
                                                             serv);
                else
