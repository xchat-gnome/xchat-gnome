...
static char *ignore_read_next_entry(char *my_cfg, struct ignore *ignore)
{
        char tbuf[1024];
...
static gboolean flood_autodialog_timeout(gpointer data)
{
        prefs.autodialog = 1;
        return FALSE;
}
...
int flood_check(char *nick, char *ip, server *serv, session *sess, int what) /*0=ctcp  1=priv */
{
        ...
        if (what == 0) {
                if (serv->ctcp_last_time == 0) /*first ctcp in this server */
                {
                        ...
                        if (difftime(current_time, serv->ctcp_last_time) <
                            prefs.ctcp_time_limit) /*if we got the ctcp in the seconds limit */
                        {
                                serv->ctcp_counter++;
                                if (serv->ctcp_counter ==
                                    prefs
                                        .ctcp_number_limit) /*if we reached the maximun numbers of
                                                               ctcp in the seconds limits */
                                {
                                        serv->ctcp_last_time =
                                            current_time; /*we got the flood, restore all the vars
                                                             for next one */
                                        serv->ctcp_counter = 0;
                                        for (i = 0; i < 128; i++)
                                                if (ip[i] == '@')
                                        ...
                                        /*FIXME: only ignore ctcp or all?, its ignoring ctcps for
                                         * now */
                ...
                if (serv->msg_last_time == 0) {
                        ...
                        if (difftime(current_time, serv->msg_last_time) < prefs.msg_time_limit) {
                                serv->msg_counter++;
                                if (serv->msg_counter ==
                                    prefs.msg_number_limit) /*if we reached the maximun numbers of
                                                               ctcp in the seconds limits */
                                {
                                        snprintf(buf,
                                                 sizeof(buf),
                                                 _("You are being MSG flooded from %s, setting "
                                                   "gui_auto_open_dialog OFF.\n"),
                                                 ip);
                                        PrintText(sess, buf);
                                        serv->msg_last_time =
                                            current_time; /*we got the flood, restore all the vars
                                        ...
                                        if (prefs.autodialog) {
                                                /*FIXME: only ignore ctcp or all?, its ignoring
                                                 * ctcps for now */
                                                prefs.autodialog = 0;
