...
static gboolean notify_do_network(struct notify *notify, server *serv)
{
...
static struct notify_per_server *notify_find(server *serv, char *nick)
{
        GSList *list = notify_list;
        struct notify_per_server *servnot;
        struct notify *notify;
...
static void notify_announce_offline(server *serv, struct notify_per_server *servnot, char *nick,
                                    int quiet)
{
        session *sess;
        ...
        if (!quiet)
                EMIT_SIGNAL(XP_TE_NOTIFYOFFLINE,
                            sess,
                            nick,
                            serv->servername,
...
static void notify_announce_online(server *serv, struct notify_per_server *servnot, char *nick)
{
        session *sess;
        ...
        EMIT_SIGNAL(XP_TE_NOTIFYONLINE,
                    sess,
                    nick,
                    serv->servername,
                    server_get_network(serv, TRUE),
...
static void notify_watch_all(struct notify *notify, int add)
{
        server *serv;
        ...
        while (list) {
                ...
                while (*word[i]) {
                        ...
                        /* FIXME: word[] is only a 32 element array, limits notify list to
                           about 27 people */
                        if (i > PDIWORDS - 5) {
                                /*fprintf (stderr, _("*** XCHAT WARNING: notify list too
                                 * large.\n"));*/
                                break;
                        }
                }
