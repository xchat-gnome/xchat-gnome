...
static void irc_nickserv(server *serv, char *cmd, char *arg1, char *arg2, char *arg3)
{
...
static void irc_set_back(server *serv)
{
...
static void process_numeric(session *sess, int n, char *word[], char *word_eol[], char *text)
{
        ...
        case 1:
                ...
                if ((strncmp(word[7], "PTnet", 5) == 0) && (strncmp(word[8], "IRC", 3) == 0) &&
                    (strncmp(word[9], "Network", 7) == 0) && (strrchr(word[10], '@') != NULL)) {
                        serv->use_who = FALSE;
                ...
                /* use /NICKSERV */
                if (strcasecmp(word[7], "DALnet") == 0 || strcasecmp(word[7], "BRASnet") == 0)
                        serv->nickservtype = 1;
        ...
        case 317: {
                ...
                snprintf(outbuf,
                         ...
                         idle % 60);
                if (timestamp == 0)
                        EMIT_SIGNAL(XP_TE_WHOIS4, whois_sess, word[4], outbuf, NULL, NULL, 0);
                else {
                        tim = ctime(&timestamp);
                        tim[19] = 0; /* get rid of the \n */
                        EMIT_SIGNAL(XP_TE_WHOIS4T, whois_sess, word[4], outbuf, tim, NULL, 0);
                }
        } break;

        case 318:
                serv->inside_whois = 0;
                EMIT_SIGNAL(XP_TE_WHOIS6, whois_sess, word[4], NULL, NULL, NULL, 0);
                break;

        case 313:
        case 319:
                EMIT_SIGNAL(XP_TE_WHOIS2, whois_sess, word[4], word_eol[5] + 1, NULL, NULL, 0);
                break;
	...
	case 338:  /* Undernet Real user@host, Real IP */
        ...
        default:
                ...
                if (serv->inside_whois && word[4][0]) {
                        ...
                        EMIT_SIGNAL(XP_TE_WHOIS_SPECIAL,
                                    whois_sess,
                                    word[4],
                                    (word_eol[5][0] == ':') ? word_eol[5] + 1 : word_eol[5],
                                    word[2],
...
static void process_named_msg(session *sess, char *type, char *word[], char *word_eol[])
{
        server *serv = sess->server;
...
static void process_named_servermsg(session *sess, char *buf, char *word_eol[])
{
