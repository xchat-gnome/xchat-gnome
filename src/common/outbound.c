...
void process_data_init(char *buf, char *cmd, char *word[], char *word_eol[], gboolean handle_quotes,
...
static int cmd_allservers(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        GSList *list;
        server *serv;
...
static int cmd_chanopt(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int state;
...
static int cmd_charset(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        server *serv = sess->server;
...
static int cmd_dehop(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int i = 2;
...
static int cmd_mdehop(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static void menu_add(char *path, char *label, char *cmd, char *ucmd, int pos, int state, int markup,
                     int enable, int mod, int key, char *group, char *icon)
{
        menu_entry *me;
...
static int cmd_menu(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int idx = 2;
        int len;
        int pos = 0xffff;
...
{
        if (!user->op && !user->me)
                data->sess->server->p_kick(data->sess->server,
                                           data->sess->channel,
                                           user->nick,
                                           data->reason);
        return TRUE;
...
static gboolean exec_data(GIOChannel *source, GIOCondition condition, struct nbexec *s)
{
...
static int cmd_flushq(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static int cmd_getint(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        getvalinfo *info;
...
static int cmd_getfile(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int idx = 2;
        int flags = 0;
...
static int cmd_getstr(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        getvalinfo *info;
...
static int cmd_ghost(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        if (word[3][0]) {
...
static int cmd_gui(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static int cmd_help(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int i = 0, longfmt = 0;
        char *helpcmd = "";
        GSList *list;
...
static int cmd_id(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        if (word[2][0]) {
...
static int cmd_killall(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        xchat_exit();
        return 2;
}

static int cmd_lagcheck(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        lag_check();
        return TRUE;
}

static void lastlog(session *sess, char *search, gboolean regexp)
{
...
static int cmd_list(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static int cmd_mode(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static int cmd_mop(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        ...
        if (*nick) {
                if (*msg) {
                        if (strcmp(nick, ".") == 0) { /* /msg the last nick /msg'ed */
                                ...
                                safe_strcpy(sess->lastnick, nick, NICKLEN); /* prime the last nick
                                                                               memory */
                        }
                        ...
                        if (newsess)
                                inbound_chanmsg(newsess->server,
                                                NULL,
                                                newsess->channel,
                                                newsess->server->nick,
                                                msg,
...
static int cmd_recv(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        if (*word_eol[2]) {
...
static int cmd_send(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        guint32 addr;
        socklen_t len;
...
static int cmd_setcursor(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int delta = FALSE;
...
static int cmd_settext(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        fe_set_inputbox_contents(sess, word_eol[2]);
        return TRUE;
}

static int cmd_splay(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        if (*word[2]) {
...
static int parse_irc_url(char *url, char *server_name[], char *port[], char *channel[],
                         int *use_ssl)
{
        char *co;
#ifdef USE_OPENSSL
...
static int cmd_tray(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static int cmd_unload(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
#ifdef USE_PLUGIN
...
static int cmd_url(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        if (word[2][0]) {
                char *server_name = NULL;
                char *port = NULL;
...
static int cmd_uselect(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int idx = 2;
        int clear = TRUE;
        int scroll = FALSE;
...
static int cmd_userlist(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
...
static int cmd_hop(struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
        int i = 2;
...
const struct commands xc_cmds[] =
    { { "ADDBUTTON",
      ...
      { "BAN", cmd_ban, 1, 1, 1, N_("BAN <mask> [<bantype>], bans everyone matching the mask from "
                                    "the current channel. If they are already on the channel this "
                                    "doesn't kick them (needs chanop)") },
      { "CHANOPT",
        cmd_chanopt,
        0,
        0,
      ...
      { "CLEAR",
        ...
        0,
        1,
        N_("CLEAR [ALL|HISTORY], Clears the current text window or command history") },
      ...
      { "CTCP", cmd_ctcp, 1, 0, 1, N_("CTCP <nick> <message>, send the CTCP message to nick, "
      ...
      { "DEHOP", cmd_dehop, 1, 1, 1, N_("DEHOP <nick>, removes chanhalf-op status from the nick on "
      ...
      { "DEOP", cmd_deop, 1, 1, 1, N_("DEOP <nick>, removes chanop status from the nick on the "
                                      "current channel (needs chanop)") },
      { "DEVOICE", cmd_devoice, 1, 1, 1, N_("DEVOICE <nick>, removes voice status from the nick on "
                                            "the current channel (needs chanop)") },
...
#ifndef WIN32
      { "EXEC", cmd_exec, 0, 0, 1, N_("EXEC [-o] <command>, runs the command. If -o flag is used "
                                      "then output is sent to current channel, else is printed to "
                                      "current text box") },
#ifndef __EMX__
      { "EXECCONT", cmd_execc, 0, 0, 1, N_("EXECCONT, sends the process SIGCONT") },
#endif
      { "EXECKILL", cmd_execk, 0, 0, 1, N_("EXECKILL [-9], kills a running exec in the current "
                                           "session. If -9 is given the process is SIGKILL'ed") },
#ifndef __EMX__
      ...
      { "INVITE", cmd_invite, 1, 0, 1, N_("INVITE <nick> [<channel>], invites someone to a "
                                          "channel, by default the current channel (needs "
                                          "chanop)") },
      ...
      { "ME", cmd_me, 0, 0, 1, N_("ME <action>, sends the action to the current channel (actions "
      ...
      { "MENU",
        ...
        0,
        1,
        "MENU [-eX] [-i<ICONFILE>] [-k<mod>,<key>] [-m] [-pX] [-r<X,group>] [-tX] {ADD|DEL} <path> "
      ...
      { "NAMES", cmd_names, 1, 0, 1, N_("NAMES, Lists the nicks on the current channel") },
      { "NCTCP", cmd_nctcp, 1, 0, 1, N_("NCTCP <nick> <message>, Sends a CTCP notice") },
      { "NEWSERVER", cmd_newserver, 0, 0, 1, N_("NEWSERVER [-noconnect] <hostname> [<port>]") },
      { "NICK", cmd_nick, 0, 0, 1, N_("NICK <nickname>, sets your nick") },

      { "NOTICE", cmd_notice, 1, 0, 1, N_("NOTICE <nick/channel> <message>, sends a notice. "
                                          "Notices are a type of message that should be auto "
                                          "reacted to") },
      { "NOTIFY", cmd_notify, 0, 0, 1, N_("NOTIFY [-n network1[,network2,...]] [<nick>], displays "
                                          "your notify list or adds someone to it") },
      { "OP", cmd_op, 1, 1, 1, N_("OP <nick>, gives chanop status to the nick (needs chanop)") },
      { "PART",
        cmd_part,
        1,
        1,
      ...
      { "PING", cmd_ping, 1, 0, 1, N_("PING <nick | channel>, CTCP pings nick or channel") },
      { "QUERY",
        cmd_query,
        0,
        0,
      ...
      { "QUIT", cmd_quit, 0, 0, 1, N_("QUIT [<reason>], disconnects from the current server") },
      { "QUOTE", cmd_quote, 1, 0, 1, N_("QUOTE <text>, sends the text in raw form to the server") },
#ifdef USE_OPENSSL
      { "RECONNECT", cmd_reconnect, 0, 0, 1, N_("RECONNECT [-ssl] [<host>] [<port>] [<password>], "
                                                "Can be called just as /RECONNECT to reconnect to "
                                                "the current server or with /RECONNECT ALL to "
                                                "reconnect to all the open servers") },
#else
      { "RECONNECT", cmd_reconnect, 0, 0, 1, N_("RECONNECT [<host>] [<port>] [<password>], Can be "
                                                "called just as /RECONNECT to reconnect to the "
                                                "current server or with /RECONNECT ALL to "
                                                "reconnect to all the open servers") },
#endif
      { "RECV",
        cmd_recv,
...
#endif
#ifdef USE_OPENSSL
      { "SERVER", cmd_server, 0, 0, 1, N_("SERVER [-ssl] <host> [<port>] [<password>], connects to "
                                          "a server, the default port is 6667 for normal "
      ...
      { "UNBAN", cmd_unban, 1, 1, 1, N_("UNBAN <mask> [<mask>...], unbans the specified masks.") },
...
static struct commands *find_internal_command(char *name)
{
        ...
        return bsearch(name,
                       xc_cmds,
                       (sizeof(xc_cmds) / sizeof(xc_cmds[0])) - 1,
                       sizeof(xc_cmds[0]),
                       command_compare);
}
...
static void perform_nick_completion(struct session *sess, char *cmd, char *tbuf)
{
        int len;
...
static void handle_say(session *sess, char *text, int check_spch)
{
        ...
        if (!sess->channel[0] || sess->type == SESS_SERVER || sess->type == SESS_NOTICES ||
...
static int handle_user_input(session *sess, char *text, int history, int nocommand)
{
        ...
        if (prefs.cmdchar[0] == '/') {
                int i;
                const char *unix_dirs[] = { "/bin/",  "/boot/", "/dev/",        "/etc/",
