...
static void populate(PreferencesPageNetworks *page)
{
        GtkTreeIter iter;
        ircnet *net;
        GSList *netlist;
...
static void add_clicked(GtkWidget *button, PreferencesPageNetworks *page)
{
        IrcNetwork *n;
        IrcNetworkEditor *e;
...
static void edit_clicked(GtkWidget *button, PreferencesPageNetworks *page)
{
        IrcNetwork *n;
        IrcNetworkEditor *e;
        ircnet *net;
...
static void edit_activated(GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
                           PreferencesPageNetworks *page)
{
        GtkTreeIter iter;
...
static void remove_clicked(GtkWidget *button, PreferencesPageNetworks *page)
{
        GtkTreeSelection *select;
        GtkTreeIter iter;
        GtkTreeModel *model;
        ...
        dialog = gtk_message_dialog_new(NULL,
                                        ...
                                        GTK_BUTTONS_NONE,
                                        _("Really remove network \"%s\" and all of its servers?"),
                                        net->name);
...
static void selection_changed(GtkTreeSelection *selection, PreferencesPageNetworks *page)
{
...
#undef GW
        ...
        PREFERENCES_PAGE(page)
            ->icon =
            gtk_widget_render_icon(page->network_edit, GTK_STOCK_NETWORK, GTK_ICON_SIZE_MENU, NULL);
        ...
        page->sort_model = GTK_TREE_MODEL_SORT(
            gtk_tree_model_sort_new_with_model(GTK_TREE_MODEL(page->network_store)));
        gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(page->sort_model),
                                             0,
                                             GTK_SORT_ASCENDING);
        gtk_tree_view_set_model(GTK_TREE_VIEW(page->network_list),
                                GTK_TREE_MODEL(page->sort_model));
        ...
        g_signal_connect(G_OBJECT(page->network_list),
                         "row-activated",
                         G_CALLBACK(edit_activated),
                         page);
        ...
        g_signal_connect(G_OBJECT(page->network_remove),
                         "clicked",
...
static void preferences_page_networks_init(PreferencesPageNetworks *page)
{
}

static void preferences_page_networks_dispose(GObject *object)
{
...
static void preferences_page_networks_class_init(PreferencesPageNetworksClass *klass)
{
