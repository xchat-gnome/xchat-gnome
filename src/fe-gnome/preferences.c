...
static void showcolors_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                               gpointer data);
static void colorize_nicknames_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                                       gpointer data);
...
static void string_preference_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                                      gpointer user_data)
{
                            ...
                            GConfClientNotifyFunc callback, gpointer preference)
{
        ...
        gconf_client_notify_add(client, entry.key, callback, preference, NULL, NULL);
}
...
void load_preferences(void)
{
        ...
        hook_preference(client,
                        entry,
                        "/apps/xchat/irc/realname",
                        string_preference_changed,
                        prefs.realname);
        hook_preference(client,
                        entry,
        ...
        hook_preference(client,
                        ...
                        string_preference_changed,
                        prefs.partreason);
        hook_preference(client, entry, "/apps/xchat/irc/showcolors", showcolors_changed, NULL);
        hook_preference(client,
                        entry,
                        "/apps/xchat/irc/colorize_nicknames",
                        colorize_nicknames_changed,
                        NULL);
        hook_preference(client, entry, "/apps/xchat/irc/color_scheme", colors_changed, NULL);
...
void set_nickname(const gchar *text)
{
        if (NULL == text) {
                return;
        }
...
{
        prefs.stripcolor = (unsigned int)(!gconf_client_get_bool(client, entry->key, NULL));
}

static void colorize_nicknames_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                                       gpointer data)
{
        prefs.colorednicks = (unsigned int)gconf_client_get_bool(client, entry->key, NULL);
}
