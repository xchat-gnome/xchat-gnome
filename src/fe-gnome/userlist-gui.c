...
static gboolean query_user_tooltip (GtkWidget  *widget, int x, int y, gboolean keyboard_tip, GtkTooltip *tooltip, gpointer user_data);
...
initialize_userlist (void)
{
	...
	g_signal_connect(G_OBJECT (gui.userlist), "query-tooltip", G_CALLBACK (query_user_tooltip), NULL);
...
static gboolean
query_user_tooltip (GtkWidget  *widget,
		    int x,
		    int y,
