...
static void on_pgup(GtkAccelGroup *accelgroup, GObject *arg1, guint arg2, GdkModifierType arg3,
                    gpointer data);
static void on_pgdn(GtkAccelGroup *accelgroup, GObject *arg1, guint arg2, GdkModifierType arg3,
                    gpointer data);
...
static gboolean on_main_window_window_state(GtkWidget *widget, GdkEventWindowState *event,
                                            gpointer data);

static void nickname_style_set(GtkWidget *button, GtkStyle *previous_style, gpointer data);

static void main_window_userlist_location_changed(GConfClient *client, guint cnxn_id,
...
static GtkActionEntry action_entries[] = {
        ...
        { "IRCConnect",
          ...
          "<control>N",
          NULL,
          G_CALLBACK(on_irc_connect_activate) },
        { "IRCDownloads",
          NULL,
          N_("_File Transfers"),
          "<alt>F",
          NULL,
          G_CALLBACK(on_irc_downloads_activate) },
        { "IRCQuit",
          GTK_STOCK_QUIT,
          N_("_Quit"),
          "<control>Q",
          NULL,
          G_CALLBACK(on_irc_quit_activate) },
        ...
        { "EditCut",
          ...
          "<control>X",
          NULL,
          G_CALLBACK(on_edit_cut_activate) },
        { "EditCopy",
          GTK_STOCK_COPY,
          N_("_Copy"),
          "<control>C",
          NULL,
          G_CALLBACK(on_edit_copy_activate) },
        { "EditPaste",
          GTK_STOCK_PASTE,
          N_("_Paste"),
          "<control>V",
          NULL,
          G_CALLBACK(on_edit_paste_activate) },
        { "EditPreferences",
        ...
        { "NetworkReconnect",
          ...
          "<control>R",
          NULL,
          G_CALLBACK(on_network_reconnect_activate) },
        { "NetworkDisconnect",
          GTK_STOCK_DISCONNECT,
          N_("_Disconnect"),
          "",
          NULL,
          G_CALLBACK(on_network_disconnect_activate) },
        { "NetworkClose",
          GTK_STOCK_CLOSE,
          N_("_Close"),
          "<shift><control>W",
        ...
        { "NetworkChannels",
          ...
          "<alt>C",
          NULL,
          G_CALLBACK(on_network_channels_activate) },
        ...
        { "DiscussionSave",
          ...
          "<control>S",
          NULL,
          G_CALLBACK(on_discussion_save_activate) },
        { "DiscussionLeave",
          GTK_STOCK_QUIT,
          N_("_Leave"),
          "",
          NULL,
          G_CALLBACK(on_discussion_leave_activate) },
        { "DiscussionClose",
          GTK_STOCK_CLOSE,
          N_("Cl_ose"),
          "<control>W",
        ...
        { "DiscussionFind",
          ...
          "<control>F",
          NULL,
          G_CALLBACK(on_discussion_find_activate) },
        { "DiscussionChangeTopic",
          NULL,
          N_("Change _Topic"),
          "<alt>T",
          NULL,
          G_CALLBACK(on_discussion_topic_change_activate) },
        { "DiscussionBans",
          GTK_STOCK_DIALOG_WARNING,
          N_("_Bans..."),
          "<alt>B",
          NULL,
          G_CALLBACK(on_discussion_bans_activate) },
        { "DiscussionUsers",
          NULL,
          N_("_Users"),
          "<control>U",
          NULL,
          G_CALLBACK(on_discussion_users_activate) },
...
void initialize_main_window(void)
{
        ...
        g_signal_connect(G_OBJECT(gui.main_window),
                         "delete-event",
                         G_CALLBACK(on_main_window_close),
                         NULL);
        g_signal_connect(G_OBJECT(gui.main_window),
                         "focus-in-event",
                         G_CALLBACK(on_main_window_focus_in),
                         NULL);
        g_signal_connect(G_OBJECT(gui.main_window),
                         "window-state-event",
                         G_CALLBACK(on_main_window_window_state),
                         NULL);
        ...
        gtk_window_add_accel_group(GTK_WINDOW(gui.main_window),
                                   gtk_ui_manager_get_accel_group(gui.manager));
...
#undef GW
        ...
        g_signal_connect(G_OBJECT(gui.userlist_toggle),
                         "toggled",
                         G_CALLBACK(on_users_toggled),
                         NULL);

        gtk_button_set_image(GTK_BUTTON(gui.userlist_toggle),
                             gtk_image_new_from_icon_name("xchat-gnome-users", GTK_ICON_SIZE_MENU));
        ...
        g_signal_connect(G_OBJECT(gui.nick_button),
                         "clicked",
                         G_CALLBACK(on_nickname_clicked),
                         NULL);
        g_signal_connect(G_OBJECT(gtk_bin_get_child(GTK_BIN(gui.nick_button))),
                         "style-set",
                         G_CALLBACK(nickname_style_set),
                         NULL);

        client = gconf_client_get_default();

        /* move userlist to main window if applicable */
        main_window_set_show_userlist(
            gconf_client_get_bool(client, "/apps/xchat/main_window/userlist_in_main_window", NULL));

        gconf_client_notify_add(client,
                                "/apps/xchat/main_window/userlist_in_main_window",
                                (GConfClientNotifyFunc)main_window_userlist_location_changed,
                                NULL,
                                NULL,
                                NULL);
...
void run_main_window(gboolean fullscreen)
{
        ...
        int width = gconf_client_get_int(client, "/apps/xchat/main_window/width", NULL);
        int height = gconf_client_get_int(client, "/apps/xchat/main_window/height", NULL);
        if (width == 0 || height == 0) {
                width = 800;
                height = 550;
        }
        ...
        int h = gconf_client_get_int(client, "/apps/xchat/main_window/hpane", NULL);
        if (h != 0) {
        ...
        action = gtk_action_group_get_action(gui.action_group, "ViewFullscreen");
...
static void on_sidebar_toggled(GtkToggleAction *action, gpointer data)
{
        GConfClient *client;
...
static void on_statusbar_toggled(GtkToggleAction *action, gpointer data)
{
        GConfClient *client;
        client = gconf_client_get_default();
        gconf_client_set_bool(client,
...
static void on_fullscreen_toggled(GtkToggleAction *action, gpointer data)
{
        if (gtk_toggle_action_get_active(action)) {
...
static void nickname_dialog_entry_activated(GtkEntry *entry, GtkDialog *dialog)
{
...
static void on_nickname_clicked(GtkButton *widget, gpointer user_data)
{
        ...
        GtkWidget *away_button =
            GTK_WIDGET(gtk_builder_get_object(gui.xml, "nickname dialog away"));
        ...
        g_signal_connect(G_OBJECT(entry),
                         "activate",
                         G_CALLBACK(nickname_dialog_entry_activated),
                         dialog);
...
void main_window_set_show_userlist(gboolean show_in_main_window)
{
        ...
        if (show_in_main_window) {
                ...
                gtk_widget_reparent(GTK_WIDGET(gui.userlist),
                                    GTK_WIDGET(
                                        gtk_builder_get_object(gui.xml,
                                                               "scrolledwindow_userlist_main")));
                ...
                gtk_widget_hide(
                    GTK_WIDGET(gtk_builder_get_object(gui.xml, "scrolledwindow_userlist_main")));
                gtk_widget_reparent(GTK_WIDGET(gui.userlist),
                                    GTK_WIDGET(gtk_builder_get_object(gui.xml,
                                                                      "scrolledwindow_userlist")));
