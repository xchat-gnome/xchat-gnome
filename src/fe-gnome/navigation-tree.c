...
static gint tree_iter_sort_func_nocase(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                                       gpointer data);
...
static void jump_to_discussion(GtkAccelGroup *accelgroup, GObject *arg1, guint arg2,
                               GdkModifierType arg3, gpointer data);
...
static gboolean find_session(NavModel *model, session *sess, GtkTreeIter *iter,
                             GtkTreeIter *parent);
...
static GtkActionEntry action_entries[] = {
        ...
        { "ViewPreviousNetwork",
          NULL,
          N_("Pre_vious Network"),
          "<control>Up",
          NULL,
          G_CALLBACK(go_previous_network) },
        { "ViewNextNetwork",
          NULL,
          N_("Nex_t Network"),
          "<control>Down",
          NULL,
          G_CALLBACK(go_next_network) },
        { "ViewPreviousDiscussion",
          NULL,
          N_("_Previous Discussion"),
          "<alt>Up",
          NULL,
          G_CALLBACK(go_previous_discussion) },
        { "ViewNextDiscussion",
          NULL,
          N_("_Next Discussion"),
          "<alt>Down",
          NULL,
          G_CALLBACK(go_next_discussion) },
...
static GtkToggleActionEntry toggle_action_entries[] = {
        ...
        { "NetworkAutoConnect",
          NULL,
          N_("_Auto-connect on startup"),
          "",
          NULL,
          G_CALLBACK(on_server_autoconnect),
          FALSE },

        // Discussion context menu
        { "DiscussionAutoJoin",
          NULL,
          N_("_Auto-join on connect"),
          "",
          NULL,
          G_CALLBACK(on_channel_autojoin),
          FALSE },
        { "DiscussionConfMode",
          NULL,
          N_("Show join/part messages"),
          "",
          NULL,
          G_CALLBACK(on_channel_confmode),
          FALSE }
};

GType navigation_tree_get_type(void)
{
        ...
        if (!navigation_tree_type) {
                ...
                navigation_tree_type =
                    g_type_register_static(GTK_TYPE_TREE_VIEW, "NavTree", &navigation_tree_info, 0);
        }
...
static void navigation_tree_init(NavTree *navtree)
{
        ...
        navtree->priv->selection_changed_id =
            g_signal_connect(G_OBJECT(select), "changed", G_CALLBACK(selection_changed), navtree);
        ...
        gtk_action_group_add_actions(navtree->priv->action_group,
                                     action_entries,
                                     G_N_ELEMENTS(action_entries),
                                     NULL);
        gtk_action_group_add_toggle_actions(navtree->priv->action_group,
...
static void selection_changed(GtkTreeSelection *selection, NavTree *tree)
{
        ...
        if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
                ...
                if (sess) {
                        ...
                        GtkAction *action = gtk_action_group_get_action(tree->priv->action_group,
                                                                        "DiscussionAutoJoin");
                        gtk_action_set_sensitive(action, sess->server->network != NULL);
                        gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(action),
                                                     channel_is_autojoin(sess));

                        action = gtk_action_group_get_action(tree->priv->action_group,
                                                             "DiscussionConfMode");
                        gtk_action_set_sensitive(action, sess->server->network != NULL);
                        gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(action),
                                                     !sess->hide_join_part);
...
GType navigation_model_get_type(void)
{
        ...
        if (!navigation_model_type) {
                ...
                navigation_model_type = g_type_register_static(GTK_TYPE_TREE_STORE,
                                                               "NavModel",
                                                               &navigation_model_info,
                                                               0);
        }
...
static void navigation_model_init(NavModel *navmodel)
{
        ...
        gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(navmodel),
                                        1,
                                        tree_iter_sort_func_nocase,
                                        NULL,
                                        NULL);
}
...
void navigation_model_set_current(NavModel *model, session *sess)
{
        ...
        gint refcount;
        gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, COLUMN_REFCOUNT, &refcount, -1);
...
static gboolean channel_is_autojoin(session *sess)
{
        ...
        if (network == NULL || network->autojoin == NULL ||
            g_utf8_strlen(network->autojoin, 0) == 0) {
                return FALSE;
        }
...
void navigation_tree_add_accels(NavTree *navtree, GtkWindow *window)
{
        ...
        for (int i = 0; i < 9; i++) {
                ...
                gtk_accel_group_connect(discussion_accel,
                                        GDK_KEY_1 + i,
                                        GDK_MOD1_MASK,
                                        GTK_ACCEL_VISIBLE,
                                        closure);
...
static void join_channel(GtkAction *action, gpointer data)
{
        ...
        session *sess;
        gchar *channel;
        gtk_tree_model_get(model, &iter, COLUMN_NAME, &channel, COLUMN_SESSION, &sess, -1);
...
void set_action_state(NavTree *navtree)
{
        ...
        GtkAction *discussion_save =
            gtk_ui_manager_get_action(gui.manager, "/menubar/DiscussionMenu/DiscussionSave");
        GtkAction *discussion_close =
            gtk_ui_manager_get_action(gui.manager, "/menubar/DiscussionMenu/DiscussionClose");
        GtkAction *discussion_join =
            gtk_ui_manager_get_action(gui.manager, "/ChannelPopup/DiscussionJoin");
        GtkAction *discussion_leave =
            gtk_ui_manager_get_action(gui.manager, "/ChannelPopup/DiscussionLeave");
        GtkAction *discussion_auto_join =
            gtk_ui_manager_get_action(gui.manager, "/ChannelPopup/DiscussionAutoJoin");
        GtkAction *discussion_conf_mode =
            gtk_ui_manager_get_action(gui.manager, "/ChannelPopup/DiscussionConfMode");
        GtkAction *discussion_change_topic =
            gtk_ui_manager_get_action(gui.manager, "/menubar/DiscussionMenu/DiscussionChangeTopic");
        GtkAction *discussion_users =
            gtk_ui_manager_get_action(gui.manager, "/menubar/DiscussionMenu/DiscussionUsers");
        GtkAction *discussion_bans =
            gtk_ui_manager_get_action(gui.manager, "/menubar/DiscussionMenu/DiscussionBans");
        GtkAction *discussion_find =
            gtk_ui_manager_get_action(gui.manager, "/menubar/DiscussionMenu/DiscussionFind");

        GtkAction *network_close =
            gtk_ui_manager_get_action(gui.manager, "/menubar/NetworkMenu/NetworkClose");
        GtkAction *network_reconnect =
            gtk_ui_manager_get_action(gui.manager, "/menubar/NetworkMenu/NetworkReconnect");
        GtkAction *network_disconnect =
            gtk_ui_manager_get_action(gui.manager, "/menubar/NetworkMenu/NetworkDisconnect");
        GtkAction *network_channels =
            gtk_ui_manager_get_action(gui.manager, "/menubar/NetworkMenu/NetworkChannels");
        GtkAction *network_auto_connect =
            gtk_ui_manager_get_action(gui.manager, "/NetworkPopup/NetworkAutoConnect");
        ...
        session *sess;
        gboolean connected;
        gtk_tree_model_get(model, &iter, COLUMN_SESSION, &sess, COLUMN_CONNECTED, &connected, -1);
        ...
        case SESS_CHANNEL:
                ...
                gtk_action_set_sensitive(discussion_join,
                                         (connected == FALSE && sess->server->connected == TRUE));
...
static void set_server_autoconnect(session *sess, gboolean autoconnect)
{
        gboolean current;
        ircnet *network;
...
static void set_channel_autojoin(session *sess, gboolean autojoin)
{
        gboolean current;
                ...
                for (i = 0; channels[i]; i++) {
                        if (strcmp(channels[i], sess->channel) != 0) {
                                if (new_channels == NULL) {
                                        ...
                                        tmp = new_channels;
                                        new_channels =
                                            g_strdup_printf("%s,%s", new_channels, channels[i]);
                                        g_free(tmp);
                                }
                                ...
                                if (!keys_done && keys[i]) {
                                        if (new_keys == NULL)
                                                ...
                                                tmp = new_keys;
                                                new_keys =
                                                    g_strdup_printf("%s,%s", new_keys, keys[i]);
                                                g_free(tmp);
                                        }
                                } else {
                                        keys_done = TRUE;
        ...
        if (!current && autojoin) {
                ...
                if (network->autojoin == NULL) {
                        ...
                        if (autojoins[1]) {
                                network->autojoin = g_strdup_printf("%s,%s %s",
                                                                    autojoins[0],
                                                                    sess->channel,
                                                                    autojoins[1]);
                        } else {
                                network->autojoin =
                                    g_strdup_printf("%s,%s", autojoins[0], sess->channel);
                        }
