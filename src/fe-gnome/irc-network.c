...
const char *encodings[] = { N_("UTF-8 (Unicode)"),
...
static void irc_network_finalize(GObject *object)
{
...
static void irc_network_class_init(IrcNetworkClass *klass)
{
        GObjectClass *object_class = (GObjectClass *)klass;
        object_class->finalize = irc_network_finalize;
}

static void irc_network_init(IrcNetwork *obj)
{
        if (!enctoindex) {
...
GType irc_network_get_type(void)
{
        ...
        if (!irc_network_type) {
                ...
                irc_network_type =
                    g_type_register_static(G_TYPE_OBJECT, "IrcNetwork", &irc_network_info, 0);
...
void irc_network_save(IrcNetwork *network)
{
        ...
        if (network->autoconnect)
                flags |= FLAG_AUTO_CONNECT;
        if (network->use_ssl)
                flags |= FLAG_USE_SSL;
        if (network->allow_invalid)
                flags |= FLAG_ALLOW_INVALID;
        if (network->cycle)
                flags |= FLAG_CYCLE;
        if (network->use_global)
