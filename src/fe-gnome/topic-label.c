...
static void topic_label_expand_activate(GtkExpander *expander, TopicLabel *label);
static char *topic_label_get_topic_string(const char *topic);
static void topic_label_size_allocate(GtkWidget *widget, GtkAllocation *allocation);
static void topic_label_url_activated(GtkWidget *url_label, const char *url, TopicLabel *label);
static void topic_entry_activate(GtkTextBuffer *textbuffer, GtkTextIter *arg1, gchar *text,
                                 gint len, GtkDialog *dialog);

#define TOPIC_LABEL_GET_PRIVATE(obj)                                                               \
        (G_TYPE_INSTANCE_GET_PRIVATE((obj), TOPIC_LABEL_TYPE, TopicLabelPriv))
...
static void topic_label_init(TopicLabel *label)
{
        ...
        g_signal_connect(G_OBJECT(priv->expander),
                         "activate",
                         G_CALLBACK(topic_label_expand_activate),
                         (gpointer)label);
        g_signal_connect(G_OBJECT(priv->label),
                         "activate-link",
                         G_CALLBACK(topic_label_url_activated),
                         (gpointer)label);
...
static char *topic_label_get_topic_string(const char *topic)
{
        ...
        for (i = 1; tokens[i]; i++) {
                if (url_check_word(tokens[i], strlen(tokens[i])) == WORD_URL) {
                        temp = g_strdup_printf("%s <a href=\"%s\">%s</a>",
                                               result,
                                               tokens[i],
                                               tokens[i]);
                        g_free(result);
...
void topic_label_change_current(TopicLabel *label)
{
        ...
        if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
                ...
                priv->current->server->p_topic(priv->current->server,
