...
enum { WORD_CLICK, WORD_ENTER, WORD_LEAVE, LAST_SIGNAL };
...
static inline GdkColor xtext_palette_lookup(GtkXText *xtext, int index)
{
        return xtext->palette[index];
}

static inline void xtext_set_source_color(cairo_t *cr, GdkColor *color, gdouble alpha)
{
...
static inline void xtext_draw_rectangle(GtkXText *xtext, cairo_t *cr, GdkColor *color, int x, int y,
...
static inline void xtext_draw_line(GtkXText *xtext, cairo_t *cr, GdkColor *color, int x1, int y1,
                                   int x2, int y2)
{
        cairo_save(cr);
...
static inline void xtext_draw_bg(GtkXText *xtext, int x, int y, int width, int height)
{
        cairo_t *cr;
...
static void xtext_draw_layout_line(cairo_t *cr, int x, int y, PangoLayoutLine *line)
{
        cairo_save(cr);
...
static void backend_draw_text(GtkXText *xtext, int dofill, int x, int y, char *str, int len,
                              int str_width, int is_mb)
{
...
static void gtk_xtext_init(GtkXText *xtext)
{
        ...
        xtext->vc_signal_tag = g_signal_connect(G_OBJECT(xtext->adj),
                                                ...
                                                xtext);
        {
                static const GtkTargetEntry targets[] = { { "UTF8_STRING", 0, TARGET_UTF8_STRING },
...
static void gtk_xtext_adjustment_set(xtext_buffer *buf, int fire_signal)
{
        ...
        if (buf->xtext->buffer == buf) {
                ...
                gtk_adjustment_set_page_size(adj,
                                             (allocation.height - buf->xtext->font->descent) /
                                                 buf->xtext->fontsize);
...
static void gtk_xtext_adjustment_changed(GtkAdjustment *adj, GtkXText *xtext)
{
        if (xtext->buffer->old_value != gtk_adjustment_get_value(xtext->adj)) {
                if (gtk_adjustment_get_value(xtext->adj) >=
                ...
                if (gtk_adjustment_get_value(xtext->adj) + 1 == xtext->buffer->old_value ||
                    gtk_adjustment_get_value(xtext->adj) - 1 ==
                        ...
                        if (!xtext->io_tag)
                                xtext->io_tag =
                                    g_timeout_add(REFRESH_TIMEOUT,
                                                  (GSourceFunc)gtk_xtext_adjustment_timeout,
                                                  xtext);
                }
        }
...
static void gtk_xtext_destroy(GObject *object)
{
...
static void gtk_xtext_get_preferred_width(GtkWidget *widget, gint *minimal_width,
                                          gint *natural_width)
{
        *minimal_width = *natural_width = 200;
}

static void gtk_xtext_get_preferred_height(GtkWidget *widget, gint *minimal_height,
                                           gint *natural_height)
{
...
static int gtk_xtext_find_x(GtkXText *xtext, int x, textentry *ent, int subline, int line,
                            int *out_of_bounds)
{
        int indent;
        unsigned char *str;
...
static void gtk_xtext_selection_up(GtkXText *xtext, textentry *start, textentry *end,
                                   int start_offset)
{
...
static void gtk_xtext_selection_down(GtkXText *xtext, textentry *start, textentry *end,
                                     int end_offset)
{
        /* render all the complete lines */
        if (end->prev == start)
...
static void gtk_xtext_selection_render(GtkXText *xtext, textentry *start_ent, int start_offset,
                                       textentry *end_ent, int end_offset)
{
        ...
        /* marking downward? */
                ...
                while (ent) {
                        ...
                        if (ent == end_ent) {
                                gtk_xtext_selection_down(xtext,
                                                         ent,
...
static void gtk_xtext_selection_draw(GtkXText *xtext, GdkEventMotion *event, gboolean render)
{
        ...
        if (xtext->buffer->last_ent_start == ent_start && xtext->buffer->last_ent_end == ent_end &&
...
static void gtk_xtext_selection_update(GtkXText *xtext, GdkEventMotion *event, int p_y,
                                       gboolean render)
{
        int win_height;
        int moved;
        ...
        if (p_y < 0 && gtk_adjustment_get_value(xtext->adj) >= 0) {
                if (!xtext->scroll_tag)
                        xtext->scroll_tag =
                            g_timeout_add(100, (GSourceFunc)gtk_xtext_scrollup_timeout, xtext);
                return;
        }
        ...
        if (p_y > win_height &&
            gtk_adjustment_get_value(xtext->adj) <
                ...
                if (!xtext->scroll_tag)
                        xtext->scroll_tag =
                            g_timeout_add(100, (GSourceFunc)gtk_xtext_scrolldown_timeout, xtext);
                return;
        }
...
static char *gtk_xtext_get_word(GtkXText *xtext, int x, int y, textentry **ret_ent, int *ret_off,
                                int *ret_len)
{
        textentry *ent;
        int offset;
...
static gboolean gtk_xtext_motion_notify(GtkWidget *widget, GdkEventMotion *event)
{
        ...
        if (!word || (xtext->current_word != NULL && strcmp(word, xtext->current_word) != 0)) {
                if (xtext->current_word) {
                        g_signal_emit(G_OBJECT(xtext),
        ...
        if (word) {
                if (xtext->urlcheck_function(GTK_WIDGET(xtext), word, len) > 0) {
                        ...
                        if (!xtext->current_word || strcmp(word, xtext->current_word) != 0) {
...
static gboolean gtk_xtext_button_release(GtkWidget *widget, GdkEventButton *event)
{
        ...
        if (event->button == 1) {
                ...
                if (xtext->select_start_x == event->x && xtext->select_start_y == event->y &&
                    xtext->buffer->last_ent_start) {
...
static char *gtk_xtext_selection_get_text(GtkXText *xtext, int *len_ret)
{
        ...
        *pos = 0;
        ...
        if (xtext->color_paste) {
                ...
                stripped = (char *)
                    gtk_xtext_strip_color((unsigned char *)txt, strlen(txt), NULL, &len, 0, FALSE);
                free(txt);
        }
...
static void gtk_xtext_selection_get(GtkWidget *widget, GtkSelectionData *selection_data_ptr,
...
static gboolean gtk_xtext_scroll(GtkWidget *widget, GdkEventScroll *event)
{
        ...
        {
                new_value = gtk_adjustment_get_value(xtext->adj) -
                            (gtk_adjustment_get_page_increment(xtext->adj) / 10);
        ...
        } else if (event->direction == GDK_SCROLL_DOWN) /* mouse wheel pageDn */
        {
                new_value = gtk_adjustment_get_value(xtext->adj) +
                            (gtk_adjustment_get_page_increment(xtext->adj) / 10);
                if (new_value > (gtk_adjustment_get_upper(xtext->adj) -
                                 gtk_adjustment_get_page_size(xtext->adj)))
                        new_value = gtk_adjustment_get_upper(xtext->adj) -
                                    gtk_adjustment_get_page_size(xtext->adj);
                gtk_adjustment_set_value(xtext->adj, new_value);
        }
...
static void gtk_xtext_class_init(GtkXTextClass *class)
{
        ...
        xtext_signals[WORD_CLICK] = g_signal_new("word_click",
                                                 ...
                                                 G_TYPE_POINTER,
                                                 G_TYPE_POINTER);
        xtext_signals[WORD_ENTER] = g_signal_new("word_enter",
                                                 G_TYPE_FROM_CLASS(widget_class),
                                                 G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                                                 ...
                                                 1,
                                                 G_TYPE_POINTER);
        xtext_signals[WORD_LEAVE] = g_signal_new("word_leave",
                                                 G_TYPE_FROM_CLASS(widget_class),
                                                 G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
...
static int gtk_xtext_text_width(GtkXText *xtext, unsigned char *text, int len, int *mb_ret)
{
        ...
        new_buf = gtk_xtext_strip_color(text,
                                        len,
                                        xtext->scratch_buffer,
                                        &new_len,
                                        &mb,
                                        !xtext->ignore_hidden);
...
static int gtk_xtext_render_flush(GtkXText *xtext, int x, int y, unsigned char *str, int len,
                                  int is_mb)
{
...
static int gtk_xtext_render_str(GtkXText *xtext, int y, textentry *ent, unsigned char *str, int len,
                                int win_width, int indent, int line, int left_only, int *x_size_ret)
{
                     ...
                     xtext->nc < 3 && !xtext->parsing_backcolor)) {
                        ...
                        if (str[i] == ',') {
                                ...
                                if (xtext->nc) {
                                        ...
                                        if (!mark)
                                                xtext->text_fg =
                                                    xtext_palette_lookup(xtext, col_num);
                                }
                        } else {
                        ...
                        case ATTR_REVERSE:
                                ...
                                if (!mark) {
                                        xtext->text_fg =
                                            xtext_palette_lookup(xtext, xtext->col_fore);
                                        xtext->text_bg =
                                            xtext_palette_lookup(xtext, xtext->col_back);
                                }
                                if (xtext->col_back != XTEXT_BG)
                                        xtext->backcolor = TRUE;
                                else
        ...
        if (!left_only && !xtext->dont_render) {
                ...
                if (!xtext->skip_border_fills && xtext->clip_x2 >= x) {
                        ...
                        xtext_draw_bg(xtext,
                                      ...
                                      MIN(xtext->clip_x2 - xx,
                                          (win_width + MARGIN) - xx), /* width */
...
static void gtk_xtext_render_stamp(GtkXText *xtext, textentry *ent, char *text, int len, int line,
                                   int win_width)
{
        textentry tmp_ent;
        int jo, ji, hs;
        ...
        gtk_xtext_render_str(xtext, y, ent, (guchar *)text, len, win_width, 2, line, TRUE, &xsize);
...
static int gtk_xtext_render_line(GtkXText *xtext, textentry *ent, int line, int lines_max,
                                 int subline, int win_width)
{
        unsigned char *str;
        ...
        do {
                ...
                y = (xtext->fontsize * line) + xtext->font->ascent - xtext->pixel_offset;
                if (!subline) {
                        if (!gtk_xtext_render_str(
                                xtext, y, ent, str, len, win_width, indent, line, FALSE, NULL)) {
                                /* small optimization */
                                gtk_xtext_draw_marker(xtext,
                                                      ent,
                                                      y -
                                                          xtext->fontsize *
                                                              (taken + start_subline + 1));
                                return ent->lines_taken - subline;
                        }
                } else {
                        xtext->dont_render = TRUE;
                        gtk_xtext_render_str(xtext,
                                             y,
                                             ent,
                                             str,
                                             len,
...
static void gtk_xtext_recalc_widths(xtext_buffer *buf, int do_str_width)
{
        ...
        while (ent) {
                if (do_str_width) {
                        ent->str_width =
                            gtk_xtext_text_width(buf->xtext, ent->str, ent->str_len, NULL);
                }
                if (ent->left_len != -1) {
                        ent->indent =
                            (buf->indent -
                             gtk_xtext_text_width(buf->xtext, ent->str, ent->left_len, NULL)) -
...
textentry *gtk_xtext_search(GtkXText *xtext, const gchar *text, textentry *start,
                            gboolean case_match, gboolean backward)
{
        ...
        if (ent) {
                ...
                if (!gtk_xtext_check_ent_visibility(xtext, ent, 0)) {
                        ...
                        while (line > gtk_adjustment_get_upper(xtext->adj) -
                                          gtk_adjustment_get_page_size(xtext->adj))
                                line--;
...
static void gtk_xtext_append_entry(xtext_buffer *buf, textentry *ent, time_t stamp)
{
        ...
        if (buf->reset_marker_pos || ((buf->marker_pos == NULL || buf->marker_seen) &&
                ...
                if (!buf->xtext->add_io_tag) {
                        ...
                        buf->xtext->add_io_tag =
                            g_timeout_add(REFRESH_TIMEOUT * 2,
                                          (GSourceFunc)gtk_xtext_render_page_timeout,
                                          buf->xtext);
                }
...
void gtk_xtext_append_indent(xtext_buffer *buf, unsigned char *left_text, int left_len,
                             unsigned char *right_text, int right_len, time_t stamp)
{
        textentry *ent;
        unsigned char *str;
        int space;
...
int gtk_xtext_lastlog(xtext_buffer *out, xtext_buffer *search_area, int (*cmp_func)(char *, void *),
                      void *userdata)
{
...
void gtk_xtext_copy_selection(GtkXText *xtext)
{
        ...
        if (str) {
                GtkClipboard *clipboard =
                    gtk_widget_get_clipboard(GTK_WIDGET(xtext), GDK_SELECTION_CLIPBOARD);
