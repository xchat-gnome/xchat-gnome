...
enum { NEW_PLUGIN_PAGE, REMOVE_PLUGIN_PAGE, LAST_SIGNAL };

G_DEFINE_TYPE(PreferencesPagePlugins, preferences_page_plugins, PREFERENCES_PAGE_TYPE)
static guint signals[LAST_SIGNAL];

static void fe_plugin_add(char *filename)
{
        GtkTreeIter iter;
        ...
        gtk_list_store_set(pageref->plugin_store,
                           &iter,
                           COL_NAME,
                           name,
                           COL_VERSION,
...
static gint filename_test(gconstpointer a, gconstpointer b)
{
        return strcmp((char *)a, (char *)b);
}

static gboolean get_plugin_infos(gchar *filename, gchar **name, gchar **desc, gchar **version,
                                 PreferencesPage **page_plugin)
{
        ...
        handle = g_module_open(filename, 0);
        if (handle != NULL &&
            g_module_symbol(handle, "xchat_plugin_get_preferences_page", &get_pref_func) &&
...
static void preferences_page_plugins_add_page(PreferencesPagePlugins *page, gchar *filename,
                                              GtkTreeIter *iter)
{
        PreferencesPage *page_plugin;
...
static void preferences_page_plugins_remove_page(PreferencesPagePlugins *page, GtkTreeIter *iter)
{
        PreferencesPage *page_plugin;
...
static void load_unload(char *filename, gboolean loaded, PreferencesPagePlugins *page,
                        GtkTreeIter iter)
{
        GConfClient *client;
        ...
        if (loaded) {
                ...
                if (err == 1) {
                        ...
                        if ((removed_plugin =
                                 g_slist_find_custom(enabled_plugins, filename, &filename_test)) !=
                            NULL) {
                                enabled_plugins =
                                    g_slist_delete_link(enabled_plugins, removed_plugin);
                        }

                } else {
                        gchar *errmsg =
                            g_strdup_printf(_("An error occurred unloading %s"), filename);
...
static void load_toggled(GtkCellRendererToggle *toggle, gchar *pathstr,
                         PreferencesPagePlugins *page)
{
        GtkTreePath *path;
        GtkTreeIter iter;
        gchar *filename;
        ...
        if (gtk_tree_model_get_iter(GTK_TREE_MODEL(page->plugin_store), &iter, path)) {
                gtk_tree_model_get(GTK_TREE_MODEL(page->plugin_store),
                                   &iter,
                                   COL_FILENAME,
                                   &filename,
                                   COL_LOADED,
...
static void open_plugin_clicked(GtkButton *button, PreferencesPagePlugins *page)
{
...
static void remove_plugin_clicked(GtkButton *button, PreferencesPagePlugins *page)
{
...
static void selection_changed(GtkTreeSelection *select, PreferencesPagePlugins *page)
{
        gtk_widget_set_sensitive(page->plugins_remove,
                                 gtk_tree_selection_get_selected(select, NULL, NULL));
}
...
static gboolean add_plugin_page_if_loaded(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                          PreferencesPagePlugins *page)
{
        gboolean loaded;
        gchar *filename;
...
static gboolean remove_page_if_exist(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                     PreferencesPagePlugins *page)
{
...
#undef GW
        ...
        PREFERENCES_PAGE(page)
            ->icon = gtk_icon_theme_load_icon(theme, "xchat-gnome-plugin", 16, 0, NULL);
         ...
         * file,          loaded*/
        page->plugin_store = gtk_list_store_new(8,
                                                G_TYPE_STRING,
                                                G_TYPE_STRING,
                                                G_TYPE_STRING,
                                                G_TYPE_STRING,
        ...
        page->load_column = gtk_tree_view_column_new_with_attributes(_("Enable"),
                                                                     page->load_renderer,
                                                                     "active",
                                                                     COL_LOADED,
                                                                     NULL);
        page->text_column = gtk_tree_view_column_new_with_attributes(_("Plugin"),
                                                                     page->text_renderer,
                                                                     "markup",
                                                                     COL_DISPLAY,
                                                                     NULL);
        ...
        g_signal_connect(G_OBJECT(page->plugins_open),
                         "clicked",
                         G_CALLBACK(open_plugin_clicked),
                         page);
        g_signal_connect(G_OBJECT(page->plugins_remove),
                         "clicked",
                         G_CALLBACK(remove_plugin_clicked),
                         page);
        g_signal_connect(G_OBJECT(select), "changed", G_CALLBACK(selection_changed), page);
        g_signal_connect(G_OBJECT(page->plugins_list),
                         "row-activated",
...
static void preferences_page_plugins_init(PreferencesPagePlugins *page)
{
}
...
void preferences_page_plugins_check_plugins(PreferencesPagePlugins *page)
{
        ...
        gtk_tree_model_foreach(GTK_TREE_MODEL(page->plugin_store),
                               (GtkTreeModelForeachFunc)add_plugin_page_if_loaded,
                               page);
}

static void preferences_page_plugins_dispose(GObject *object)
{
        PreferencesPagePlugins *page = (PreferencesPagePlugins *)object;

        if (page->plugin_store) {
                gtk_tree_model_foreach(GTK_TREE_MODEL(page->plugin_store),
                                       (GtkTreeModelForeachFunc)remove_page_if_exist,
                                       page);
...
static void preferences_page_plugins_class_init(PreferencesPagePluginsClass *klass)
{
