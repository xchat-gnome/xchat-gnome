...
static void gconf_entry_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                                GtkEntry *gtkentry)
{
        gchar *text;
...
static void gconf_bool_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
...
static void gconf_font_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
...
static void sysfonts_changed(GtkToggleButton *toggle, PreferencesPageIrc *page)
{
        gtk_widget_set_sensitive(page->font_selection, !gtk_toggle_button_get_active(toggle));
}

static void highlight_selection_changed(GtkTreeSelection *select, PreferencesPageIrc *page)
{
...
static void save_highlight(PreferencesPageIrc *page)
{
        GtkTreeIter iter;
...
static void highlight_add(GtkButton *button, PreferencesPageIrc *page)
{
        GtkTreeIter iter;
        GtkTreePath *path;
        ...
        gtk_tree_view_set_cursor(GTK_TREE_VIEW(page->highlight_list),
                                 path,
                                 page->highlight_column,
                                 TRUE);
        gtk_tree_path_free(path);
}

static void highlight_edit(GtkButton *button, PreferencesPageIrc *page)
{
...
static void highlight_remove(GtkButton *button, PreferencesPageIrc *page)
{
        GtkTreeSelection *select;
        GtkTreeIter iter;
        GtkTreeModel *model;
...
static void highlight_edited(GtkCellRendererText *renderer, gchar *arg1, gchar *newtext,
                             PreferencesPageIrc *page)
{
...
static void highlight_canceled(GtkCellRendererText *renderer, PreferencesPageIrc *page)
{
        GtkTreeSelection *selection;
        GtkTreeModel *model;
        GtkTreeIter iter;
...
static void auto_logging_changed(GtkToggleButton *button, gpointer data)
{
        gboolean active;
        GSList *list;
        session *sess;
...
static void show_marker_changed(GtkToggleButton *button, gpointer data)
{
        gboolean active;
...
static void userlist_main_changed(GtkToggleButton *button, gpointer data)
{
        gboolean active;
        ...
        gconf_client_set_bool(client,
                              "/apps/xchat/main_window/userlist_in_main_window",
                              active,
                              NULL);
        g_object_unref(client);
}
...
#undef GW
        ...
        PREFERENCES_PAGE(page)
            ->icon = gtk_icon_theme_load_icon(theme, "preferences-system", 16, 0, NULL);
        ...
        g_signal_connect(G_OBJECT(page->nick_name),
                         "changed",
                         G_CALLBACK(entry_changed),
                         "/apps/xchat/irc/nickname");
        g_signal_connect(G_OBJECT(page->real_name),
                         "changed",
                         G_CALLBACK(entry_changed),
                         "/apps/xchat/irc/realname");
        g_signal_connect(G_OBJECT(page->quit_message),
                         "changed",
                         G_CALLBACK(entry_changed),
                         "/apps/xchat/irc/quitmsg");
        g_signal_connect(G_OBJECT(page->part_message),
                         "changed",
                         G_CALLBACK(entry_changed),
                         "/apps/xchat/irc/partmsg");
        g_signal_connect(G_OBJECT(page->away_message),
                         "changed",
                         G_CALLBACK(entry_changed),
                         "/apps/xchat/irc/awaymsg");
        g_signal_connect(G_OBJECT(page->usesysfonts),
                         "toggled",
                         G_CALLBACK(bool_changed),
                         "/apps/xchat/main_window/use_sys_fonts");
        g_signal_connect(G_OBJECT(page->usesysfonts),
                         "toggled",
                         G_CALLBACK(sysfonts_changed),
                         page);
        g_signal_connect(G_OBJECT(page->font_selection),
                         "font-set",
                         G_CALLBACK(font_changed),
                         "/apps/xchat/main_window/font");
        g_signal_connect(G_OBJECT(page->show_timestamps),
                         "toggled",
                         G_CALLBACK(bool_changed),
                         "/apps/xchat/irc/showtimestamps");
        g_signal_connect(G_OBJECT(page->auto_logging),
                         "toggled",
                         G_CALLBACK(auto_logging_changed),
                         NULL);
        g_signal_connect(G_OBJECT(page->show_marker),
                         "toggled",
                         G_CALLBACK(show_marker_changed),
                         NULL);
        g_signal_connect(G_OBJECT(page->userlist_main),
                         "toggled",
                         G_CALLBACK(userlist_main_changed),
                         NULL);
        g_signal_connect(G_OBJECT(page->highlight_add), "clicked", G_CALLBACK(highlight_add), page);
        g_signal_connect(G_OBJECT(page->highlight_edit),
                         "clicked",
                         G_CALLBACK(highlight_edit),
                         page);
        g_signal_connect(G_OBJECT(page->highlight_remove),
                         "clicked",
                         G_CALLBACK(highlight_remove),
                         page);

        page->nh[0] = gconf_client_notify_add(p->gconf,
                                              "/apps/xchat/irc/nickname",
                                              (GConfClientNotifyFunc)gconf_entry_changed,
                                              page->nick_name,
                                              NULL,
                                              NULL);
        ...
        page->nh[2] = gconf_client_notify_add(p->gconf,
                                              ...
                                              NULL,
                                              NULL);
        page->nh[3] = gconf_client_notify_add(p->gconf,
                                              "/apps/xchat/irc/partmsg",
                                              (GConfClientNotifyFunc)gconf_entry_changed,
                                              page->part_message,
                                              NULL,
                                              NULL);
        page->nh[4] = gconf_client_notify_add(p->gconf,
                                              "/apps/xchat/irc/awaymsg",
                                              (GConfClientNotifyFunc)gconf_entry_changed,
                                              page->away_message,
                                              NULL,
                                              NULL);
        page->nh[5] = gconf_client_notify_add(p->gconf,
                                              "/apps/xchat/main_window/use_sys_fonts",
                                              (GConfClientNotifyFunc)gconf_bool_changed,
                                              page->usesysfonts,
                                              NULL,
                                              NULL);
        page->nh[6] = gconf_client_notify_add(p->gconf,
                                              "/apps/xchat/main-window/font",
                                              (GConfClientNotifyFunc)gconf_font_changed,
                                              page->font_selection,
                                              NULL,
                                              NULL);
        page->nh[7] = gconf_client_notify_add(p->gconf,
                                              "/apps/xchat/irc/showtimestamps",
                                              (GConfClientNotifyFunc)gconf_bool_changed,
                                              page->show_timestamps,
                                              NULL,
                                              NULL);
        ...
        toggle = gconf_client_get_bool(p->gconf,
                                       "/apps/xchat/main_window/userlist_in_main_window",
                                       NULL);
        ...
        gtk_tree_view_set_model(GTK_TREE_VIEW(page->highlight_list),
                                GTK_TREE_MODEL(page->highlight_store));
        renderer = gtk_cell_renderer_text_new();
        g_object_set(G_OBJECT(renderer), "editable", TRUE, NULL);
        page->highlight_column =
            gtk_tree_view_column_new_with_attributes("highlight", renderer, "text", 0, NULL);
...
static void preferences_page_irc_init(PreferencesPageIrc *page)
{
}

static void preferences_page_irc_dispose(GObject *object)
{
...
static void preferences_page_irc_finalize(GObject *object)
{
        PreferencesPageIrc *page = (PreferencesPageIrc *)object;
        gint i;
        GConfClient *client;
...
static void preferences_page_irc_class_init(PreferencesPageIrcClass *klass)
{
