...
static gint check_version(guint major, guint minor, guint micro)
{
...
static void dbus_migration_remove_autoload(void)
{
        GConfClient *client;
        ...
        enabled_plugins =
            gconf_client_get_list(client, "/apps/xchat/plugins/loaded", GCONF_VALUE_STRING, NULL);
...
static void dbus_migration_check_file(void)
{
        gchar *msg;

        msg =
            g_strdup_printf(_("The way the D-Bus plugin works has changed.\nTo avoid problems, you "
                              "should remove the old plugin file.\n\n<b>Please delete %s</b>"),
                            XCHATLIBDIR "/plugins/dbus.so");
