...
static void dcc_window_class_init(DccWindowClass *klass)
{
}

static void transfer_selection_changed(GtkTreeSelection *selection, DccWindow *window)
{
...
static void transfer_stop_clicked(GtkButton *button, DccWindow *window)
{
...
static void dcc_window_init(DccWindow *window)
{
...
#undef GW
        ...
        gtk_widget_set_sensitive(window->stop_button, FALSE);
        g_signal_connect(G_OBJECT(window->stop_button),
                         "clicked",
                         G_CALLBACK(transfer_stop_clicked),
                         window);
        ...
        gtk_tree_view_set_model(GTK_TREE_VIEW(window->transfer_list),
                                GTK_TREE_MODEL(window->transfer_store));
        ...
        gtk_tree_view_column_add_attribute(window->progress_column,
                                           window->progress_cell,
                                           "value",
                                           DONE_COLUMN);
        gtk_tree_view_column_add_attribute(window->progress_column,
                                           window->progress_cell,
        ...
        gtk_tree_view_column_add_attribute(window->info_column,
                                           window->icon_cell,
                                           "gicon",
                                           ICON_COLUMN);
        ...
        gtk_tree_view_column_add_attribute(window->remaining_column,
                                           window->remaining_cell,
                                           "text",
                                           TIME_COLUMN);
        ...
        g_signal_connect(G_OBJECT(
                             gtk_tree_view_get_selection(GTK_TREE_VIEW(window->transfer_list))),
                         "changed",
                         G_CALLBACK(transfer_selection_changed),
                         window);
}

GType dcc_window_get_type(void)
{
        ...
        if (!dcc_window_type) {
                ...
                dcc_window_type =
                    g_type_register_static(GTK_TYPE_WINDOW, "DccWindow", &dcc_window_info, 0);
...
void dcc_window_add(DccWindow *window, struct DCC *dcc)
{
        ...
        if ((dcc->type == TYPE_RECV) && (dcc->dccstat == STAT_QUEUED) && (prefs.autodccsend != 1)) {
                ...
                gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog),
                                                         _("%s is attempting to send you a file "
                                                           "named \"%s\". Do you wish to accept "
                                                           "the transfer?"),
                                                         dcc->nick,
                                                         dcc->file);
        ...
        gtk_list_store_set(window->transfer_store,
                           &iter,
                           DCC_COLUMN,
                           dcc,
                           INFO_COLUMN,
...
void dcc_window_update(DccWindow *window, struct DCC *dcc)
{
        ...
        if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(window->transfer_store), &iter)) {
                do {
                        gpointer ptr;
                        gpointer icon;
                        gtk_tree_model_get(GTK_TREE_MODEL(window->transfer_store),
                                           &iter,
                                           DCC_COLUMN,
                                           &ptr,
                                           ICON_COLUMN,
                        ...
                        if (ptr == dcc) {
                                ...
                                if (dcc->dccstat == STAT_QUEUED) {
                                        ...
                                        if (dcc->type == TYPE_SEND)
                                                message = g_strdup_printf(
                                                    _("Transfer of %s to %s failed"),
                                                    dcc->file,
                                                    dcc->nick);
                                        else
                                                message = g_strdup_printf(
                                                    _("Transfer of %s from %s failed"),
                                                    dcc->file,
                                                    dcc->nick);
                                        ...
                                        if (dcc->cps == 0) {
                                                ...
                                                if (eta > 3600)
                                                        remaining_text =
                                                            g_strdup_printf(_("%.2d:%.2d:%.2d"),
                                                                            eta / 3600,
                                                                            (eta / 60) % 60,
                                                                            eta % 60);
                                                else
                                                        remaining_text =
                                                            g_strdup_printf(_("%.2d:%.2d"),
                                                                            eta / 60,
                                                                            eta % 60);
                                        }
                                }
                                ...
                                if (dcc->destfile != NULL && icon == NULL) {
                                        ...
                                        if (g_file_test(dcc->destfile, G_FILE_TEST_EXISTS) ==
                                            FALSE) {
                                                return;
                                        }
                                        ...
                                        /* Composite on an emblem which shows the direction of
                                         * transfer */
                                        if (dcc->type == 1) {
                                        ...
                                        emblemed_icon =
                                            g_emblemed_icon_new(mime_icon, direction_emblem);
