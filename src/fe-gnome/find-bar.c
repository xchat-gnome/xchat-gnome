...
static void find_bar_close_cb(GtkWidget *button, FindBar *bar);
static void find_bar_search_changed(GtkWidget *entry, FindBar *bar);
static gboolean find_bar_key_press(GtkWidget *entry, GdkEventKey *event, FindBar *bar);
static void find_bar_search(FindBar *bar, gboolean reverse);
static void find_bar_set_status(FindBar *bar, guint status);
static void find_bar_next(GtkWidget *button, FindBar *bar);
static void find_bar_previous(GtkWidget *button, FindBar *bar);
...
static void find_bar_init(FindBar *bar)
{
        ...
        bar->priv->status_wrapped =
            gtk_image_new_from_icon_name("xchat-gnome-search-wrapped", GTK_ICON_SIZE_MENU);
        bar->priv->status_notfound =
            gtk_image_new_from_stock(GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_BUTTON);
        ...
        g_signal_connect(G_OBJECT(bar->priv->entry),
                         "changed",
                         G_CALLBACK(find_bar_search_changed),
                         bar);
        g_signal_connect(G_OBJECT(bar->priv->entry),
                         "key-press-event",
                         G_CALLBACK(find_bar_key_press),
                         bar);
...
static void find_bar_search(FindBar *bar, gboolean reverse)
{
        ...
        position = conversation_panel_search(CONVERSATION_PANEL(gui.conversation_panel),
                                             text,
                                             bar->priv->last_position,
                                             FALSE,
                                             reverse);
...
static void find_bar_set_status(FindBar *bar, guint status)
{
        ...
        case STATUS_WRAPPED:
                ...
                gtk_label_set_markup(
                    GTK_LABEL(bar->priv->status_label),
        ...
        case STATUS_WRAPPED_REVERSE:
                ...
                gtk_label_set_markup(GTK_LABEL(bar->priv->status_label),
                                     _("<span foreground=\"dark grey\">Reached beginning, "
                                       "continuing from bottom</span>"));
                break;
