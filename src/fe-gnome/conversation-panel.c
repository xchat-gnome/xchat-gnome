...
static int conversation_panel_check_word(GtkWidget *xtext, char *word, int len);
static void conversation_panel_clicked_word(GtkWidget *xtext, char *word, GdkEventButton *event,
                                            ConversationPanel *panel);
static void conversation_panel_enter_word(GtkWidget *xtext, char *word, ConversationPanel *panel);
static void conversation_panel_leave_word(GtkWidget *xtext, char *word, ConversationPanel *panel);
static gboolean conversation_panel_lost_focus(GtkWidget *widget, GdkEventFocus *event,
                                              ConversationPanel *panel);
static void conversation_panel_set_font(ConversationPanel *panel);
static void conversation_panel_font_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                                            ConversationPanel *panel);
static void conversation_panel_set_background(ConversationPanel *panel);
static void conversation_panel_background_changed(GConfClient *client, guint cnxn_id,
                                                  GConfEntry *entry, ConversationPanel *panel);
static gboolean conversation_panel_query_tooltip(GtkWidget *widget, gint x, gint y,
                                                 gboolean keyboard_tooltip, GtkTooltip *tooltip);

static void timestamps_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                               xtext_buffer *buffer);

static void redundant_nickstamp_changed(GConfClient *client, guint cnxn_id, GConfEntry *entry,
                                        ConversationPanel *panel);
static void conversation_panel_print_line(ConversationPanel *panel, xtext_buffer *buffer,
                                          char *text, int len, gboolean indent, time_t timet);
static void conversation_panel_lastlog_foreach(GtkXText *xtext, char *text, fe_lastlog_info *info);

static void open_url(GtkAction *action, ConversationPanel *panel);
static void copy_text(GtkAction *action, ConversationPanel *panel);
static void send_email(GtkAction *action, ConversationPanel *panel);
static void drop_send_files(GtkAction *action, ConversationPanel *panel);
static void drop_paste_file(GtkAction *action, ConversationPanel *panel);
static void drop_paste_filename(GtkAction *action, ConversationPanel *panel);
static void drop_cancel(GtkAction *action, ConversationPanel *panel);
static void style_set_callback(GtkWidget *widget, GtkStyle *previous_style, void *data);
...
static void send_file(gpointer file, gpointer user_data);
static void on_default_copy_activate(GtkAction *action, ConversationPanel *panel);
...
static GtkActionEntry dnd_actions[] = {
        ...
        { "DropPasteFile",
          NULL,
          N_("Paste File _Contents"),
          NULL,
          NULL,
          G_CALLBACK(drop_paste_file) },
        { "DropPasteFileName",
          NULL,
          N_("Paste File_name"),
          NULL,
          NULL,
          G_CALLBACK(drop_paste_filename) },
...
static GtkActionEntry default_actions[] = {
        { "DefaultCopy",
          ...
          NULL,
          NULL,
          G_CALLBACK(on_default_copy_activate) },
};
...
static void conversation_panel_init(ConversationPanel *panel)
{
        ...
        panel->priv->scrollbar =
            gtk_scrollbar_new(GTK_ORIENTATION_VERTICAL, GTK_XTEXT(panel->priv->xtext)->adj);
        frame = gtk_frame_new(NULL);

        panel->priv->buffers = g_hash_table_new_full(g_direct_hash,
                                                     g_direct_equal,
                                                     NULL,
                                                     (GDestroyNotify)gtk_xtext_buffer_free);
        ...
        g_signal_connect(G_OBJECT(panel->priv->xtext),
                         "drag_data_received",
                         G_CALLBACK(drag_data_received),
                         panel);
        gtk_drag_dest_set(panel->priv->xtext,
...
static void conversation_panel_realize(GtkWidget *widget)
{
        ...
        gtk_xtext_set_urlcheck_function(GTK_XTEXT(panel->priv->xtext),
                                        conversation_panel_check_word);
        ...
        g_signal_connect(G_OBJECT(panel->priv->xtext),
                         "word_enter",
                         G_CALLBACK(conversation_panel_enter_word),
                         panel);
        g_signal_connect(G_OBJECT(panel->priv->xtext),
                         "word_leave",
                         G_CALLBACK(conversation_panel_leave_word),
                         panel);
        g_signal_connect(G_OBJECT(gui.main_window),
                         "focus-out-event",
                         G_CALLBACK(conversation_panel_lost_focus),
                         panel);
        g_signal_connect(G_OBJECT(gui.main_window),
                         "leave-notify-event",
                         G_CALLBACK(conversation_panel_lost_focus),
        ...
        gconf_client_notify_add(client,
                                "/apps/xchat/main_window/use_sys_fonts",
                                (GConfClientNotifyFunc)conversation_panel_font_changed,
                                panel,
                                NULL,
                                NULL);
        gconf_client_notify_add(client,
                                "/apps/xchat/main_window/font",
                                (GConfClientNotifyFunc)conversation_panel_font_changed,
                                panel,
                                NULL,
                                NULL);
        gconf_client_notify_add(client,
                                ...
                                (GConfClientNotifyFunc)conversation_panel_background_changed,
                                panel,
                                NULL,
                                NULL);
        gconf_client_notify_add(client,
        ...
        gconf_client_notify_add(client,
                                ...
                                NULL);
        gconf_client_notify_add(client,
                                "/apps/xchat/main_window/redundant_nickstamps",
                                (GConfClientNotifyFunc)redundant_nickstamp_changed,
                                panel,
                                NULL,
                                NULL);
        ...
        if (url == 0 && current_sess) {
                if (current_sess->type == SESS_DIALOG) {
                        ...
                        return WORD_DIALOG;
                            ...
                            userlist_find(current_sess, word + 1)) ||
...
gboolean conversation_panel_query_tooltip(GtkWidget *widget, gint x, gint y,
                                          gboolean keyboard_tooltip, GtkTooltip *tooltip)
{
        ...
        if (panel->priv->tooltip_nick) {
                struct User *user = userlist_find(gui.current_session, panel->priv->tooltip_nick);
                if (user != NULL) {
                               ...
                               ConversationPanel *panel)
{
        ...
        if (gtk_targets_include_uri(&target, 1)) {
                ...
                for (nb_uri = 0; uris[nb_uri] && strlen(uris[nb_uri]) > 0; nb_uri++) {
                        panel->priv->dropped_files =
                            g_slist_prepend(panel->priv->dropped_files, uris[nb_uri]);
                }
                ...
                if (gdk_drag_context_get_actions(context) == GDK_ACTION_ASK) {
                        ...
                        entry =
                            gtk_ui_manager_get_widget(gui.manager, "/DropFilePopup/DropPasteFile");
                        if (nb_uri > 1 ||
                        ...
                        entry =
                            gtk_ui_manager_get_widget(gui.manager, "/DropFilePopup/DropSendFiles");
                        if (panel->priv->current->type == SESS_CHANNEL) {
                                gtk_widget_set_sensitive(entry, FALSE);
                        } else {
                                gtk_widget_set_sensitive(entry, TRUE);
                        ...
                        if (gui.current_session->type == SESS_CHANNEL) {
                                ...
                                if (nb_uri == 1 && uri_is_text(panel->priv->dropped_files->data) &&
...
void conversation_panel_add_session(ConversationPanel *panel, struct session *sess, gboolean focus)
{
        ...
        client = gconf_client_get_default();
        gtk_xtext_set_time_stamp(buffer,
                                 gconf_client_get_bool(client,
                                                       "/apps/xchat/irc/showtimestamps",
                                                       NULL));
        notify = gconf_client_notify_add(client,
                                         "/apps/xchat/irc/showtimestamps",
                                         (GConfClientNotifyFunc)timestamps_changed,
                                         buffer,
                                         NULL,
                                         NULL);
        g_object_unref(client);
...
void conversation_panel_set_current(ConversationPanel *panel, struct session *sess)
{
        ...
        if (sess) {
                ...
                gtk_xtext_buffer_show(GTK_XTEXT(panel->priv->xtext),
                                      GTK_XTEXT(panel->priv->xtext)->orig_buffer,
                                      TRUE);
        }
                                          ...
                                          char *text, int len, gboolean indent, time_t timet)
{
        ...
        if (tab && tab < (text + len)) {
                ...
                if (!panel->priv->redundant_nickstamps &&
                    strncmp(buffer->laststamp, text, leftlen) == 0) {
                        text = tab + 1;
                        len -= leftlen;
                        ...
                        gtk_xtext_append_indent(buffer,
                                                (unsigned char *)text,
                                                leftlen,
                                                (unsigned char *)tab + 1,
                                                strlen(text) - leftlen - 1,
                                                timet);
                }
        } else {
                gtk_xtext_append_indent(buffer, 0, 0, (unsigned char *)text, len, timet);
...
static void conversation_panel_lastlog_foreach(GtkXText *xtext, char *text, fe_lastlog_info *info)
{
        if (nocasestrstr(text, (char *)info->sstr)) {
                conversation_panel_print(info->panel,
                                         info->sess,
                                         text,
                                         prefs.indent_nicks,
                                         time(NULL));
...
void conversation_panel_lastlog(ConversationPanel *panel, struct session *sess,
                                struct session *lsess, char *sstr)
{
        xtext_buffer *buffer;
        ...
        if (gtk_xtext_is_empty(buffer)) {
                conversation_panel_print(panel,
                                         lsess,
                ...
                gtk_xtext_foreach(buffer,
                                  (GtkXTextForeach)conversation_panel_lastlog_foreach,
                                  &info);
        }
}
...
void conversation_panel_page_up(ConversationPanel *panel)
{
        ...
        adj = GTK_XTEXT(panel->priv->xtext)->adj;
        end = gtk_adjustment_get_upper(adj) - gtk_adjustment_get_lower(adj) -
